...
--------------------------------------------------------
  ...
  begin
    ...
    for i in c_treqest loop
        ...
        if v_secur then
            if i.flgrecut = 'E' then
                ...
                begin
                    select count(*) into v_qtychoose
                     ...
                     where numreql  = i.numreqst
                       and codposl	= i.codpos
                       and statappl in ('51','61','62'); --- 51 => ???????????????????, 61 => ?????, 62  => ????????????????????
                exception when no_data_found then
                    v_qtychoose := null;
                end;
            ...
            if v_flgrecut <> i.flgrecut then
              ...
              obj_data.put('qtyreq', v_sumqtyreq);  -- sum ???????????????
              obj_data.put('qtyapp', v_sumqtyapp);    -- sum ?????????????
              obj_data.put('qtyintview', v_sumqtyintview);     -- sum ??????????????????
              obj_data.put('qtychoose', v_sumqtychoose);   -- sum ?????????????
              obj_data.put('qtyact', v_sumqtyact);    -- sum ?????
            ...
            obj_data.put('qtychoose', v_qtychoose);   --?????????????
    ...
    obj_data.put('qtyreq', v_totalqtyreq);  -- Total ???????????????
    obj_data.put('qtyapp', v_totalqtyapp);    -- Total ?????????????
    obj_data.put('qtyintview', v_totalqtyintview);     -- Total ??????????????????
    obj_data.put('qtychoose', v_totalqtychoose);   -- Total ?????????????
    obj_data.put('qtyact', v_totalqtyact);    -- Total ?????
    obj_row.put(to_char(v_rcnt-1),obj_data);
