      ...
      begin
        select numreqst, codemprc
         ...
         where numreqst = p_numreqst
           ...
           and codcomp  like p_codcomp || '%'
           and flgrecut in ('E','O')
           and rownum = 1;      -- << STD-ST11 | 000537-Boy-Apisit-Dev | 08/05/2024 | Fix issue 4448: #10738
      exception when no_data_found then
...
-- << STD-ST11 | 000537-Boy-Apisit-Dev | 08/05/2024 | Fix issue 4448: #10738
--  function check_statement (v_numreqst tapplinf.numreql%type,
...
--    v_folderd_appl      tfolderd.codapp%type := 'HRRC21E';
  ...
  begin
    obj_rows            := json_object_t();
    for i in c1 loop
--      if check_statement(i.numreql, i.numappl, i.codpos1, i.codpos2, i.codcompl) then -- << STD-ST11 | 000537-Boy-Apisit-Dev | 08/05/2024 | Fix issue 4448: #10738
        v_rcnt              := v_rcnt + 1;
        obj_data            := json_object_t();
        obj_data.put('coderror', '200');
...
--      end if; -- << STD-ST11 | 000537-Boy-Apisit-Dev | 08/05/2024 | Fix issue 4448: #10738
    end loop;
