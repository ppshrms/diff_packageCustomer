        ...
        if param_msg_error is null then
          begin
            ...
            if v_check = 'Y' then
              if b_staappr = 'Y' then
                begin
                select dteempmt, numlvl, codcomp, dteefpos  -- softberry || 31/05/2023 || #9359 || select dteempmt, numlvl, codcomp
                into v_dteempmt, v_numlvl, v_codcomp, v_dteefpos  -- softberry || 31/05/2023 || #9359 ||    into v_dteempmt, v_numlvl, v_codcomp
                    from temploy1
                   where codempid = v_codempid;
                exception when no_data_found then
                  param_msg_error := get_error_msg_php('HR2010', global_v_lang, 'temploy1');
                end;
                if trunc(v_dteempmt) > trunc(sysdate) then -- softberry || 15/06/2023 || #9359  || if trunc(v_dteempmt) >= trunc(sysdate) then
                  v_staemp := '0'; -- softberry || 15/06/2023 || #9359  || v_staemp := '1';
--<< softberry || 15/06/2023 || #9359
                else
                  v_staemp := '1';
-->> softberry || 15/06/2023 || #9359
                end if;
--<< softberry || 15/06/2023 || #9359
--                if trunc(v_dteefpos) > trunc(sysdate) then
--                  v_staemp := '0';
--                end if;
-->> softberry || 15/06/2023 || #9359
                if param_msg_error is null then
...
--<< softberry || 25/04/2023 || #8797
    ...
    -- softberry || 8/06/2023 || #8797 || v_codfrm_to := 'HRRC43UCC';

--<< softberry || 8/06/2023 || #8797
        begin
          select codform
            into v_codfrm_to
            from tfwmailh
           where codapp = 'HRRC43U';
        exception when others then
          v_codfrm_to := 'HRRC43UTO';
        end ;
-->> softberry || 8/06/2023 || #8797
        begin
...
-->> Apisit || 25/08/2023 || #8797
-->> Bak ||  chk_flowmail.replace_text_frmmail(v_template_to, 'tapplinf', v_rowidmail, v_subject, v_codfrm_to, null, v_func_appr, global_v_coduser, global_v_lang, v_msg_to); -- softberry || 25/04/2023 || #8797 || chk_flowmail.replace_text_frmmail(v_template_to, 'temploy1', v_rowidmail, get_label_name('HRRC43U1', global_v_lang, 130), v_codfrm_to, null, v_func_appr, global_v_coduser, global_v_lang, v_msg_to);
      chk_flowmail.replace_text_frmmail(v_template_to, 'tapplinf', v_rowidmail, v_subject, v_codfrm_to, '1', v_func_appr, global_v_coduser, global_v_lang, v_msg_to); -- softberry || 25/04/2023 || #8797 || chk_flowmail.replace_text_frmmail(v_template_to, 'temploy1', v_rowidmail, get_label_name('HRRC43U1', global_v_lang, 130), v_codfrm_to, null, v_func_appr, global_v_coduser, global_v_lang, v_msg_to);
-->> end
