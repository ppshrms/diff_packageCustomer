--------------------------------------------------------
--  DDL for Package Body HRRCATE
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "HRRCATE" is
-- last update: 25/05/2018 12:23
  procedure initial_value(json_str in clob) is
	...
	procedure getDestroy (json_str_input in clob, json_str_output out clob) is
		...
		obj_data		json_object_t;
		v_rcnt			number;
        p_codcompy      trcalert.codcompy%type;
		p_mailalno		trcalert.mailalno%type;
		p_dteeffec		trcalert.dteeffec%type;
	...
	begin
		...
		for i in 0..arr_destroy.get_size-1 loop
			...
			begin
				delete from trcalert where codcompy = p_codcompy and mailalno = p_mailalno and dteeffec = p_dteeffec;
				delete from trcparam where codcompy = p_codcompy and mailalno = p_mailalno and dteeffec = p_dteeffec;
				delete from trcasign where codcompy = p_codcompy and mailalno = p_mailalno and dteeffec = p_dteeffec;
				delete from tmailrrc where codcompy = p_codcompy and mailalno = p_mailalno and dteeffec = p_dteeffec;
			end;
		end loop;
  ...
  function get_seqno (v_codcompy varchar2, v_mailalno varchar2, v_dteeffec date) return number is
	...
	procedure saveData (json_str_input in clob, json_str_output out clob) as obj_row json_object_t;
        ...
        p_codsend               trcalert.codsend%type;
        p_code                  trcalert.syncond%type;
        p_statement             trcalert.statement%type;
		v_rcnt			        number := 0;
		...
		p_ffield		        trcparam.ffield%type;
        p_descript              trcparam.descript%type;
		p_codtable		        trcparam.codtable%type;
		p_fparam		        trcparam.fparam%type;
		p_flgdesc	            trcparam.flgdesc%type;
        p_flgdesc_b             boolean;
		p_numseq		        trcparam.numseq%type;
		v_numseq		        number := 0;
		p_flgDelete		        boolean;

        p_pfield                tmailrrc.pfield%type;
        p_pdesct                tmailrrc.pdesct%type; 
        v_flgappr               trcasign.flgappr%type;
        v_codcompap             trcasign.codcompap%type;
        v_codposap              trcasign.codposap%type;
        v_codempap              trcasign.codempap%type;
        v_message               trcasign.message%type;
        v_seqno                 trcasign.seqno%type;
        v_count_assign          number;
        v_count_report          number;
        v_flgsecu               boolean;
        v_zupdsal               varchar2(4000 char);
        v_countdup              number;
        v_staemp                temploy1.staemp%type;
        v_flg_assign            varchar2(100);
        v_tmp                   varchar2(10);
	begin
		initial_value(json_str_input);
        ...
        v_flgsecu := secur_main.secur2(p_codsend,global_v_coduser,global_v_zminlvl,global_v_zwrklvl,v_zupdsal);

        if not v_flgsecu then
            param_msg_error := get_error_msg_php('HR3007',global_v_lang,get_label_name('HRRCATE2',global_v_lang,'320'));
            json_str_output := get_response_message(null,param_msg_error,global_v_lang);
            return;             
        end if;
        ...
        if v_staemp = '9' then
            param_msg_error := get_error_msg_php('HR2101',global_v_lang,get_label_name('HRRCATE2',global_v_lang,'320'));
            json_str_output := get_response_message(null,param_msg_error,global_v_lang);
            return;          
        elsif v_staemp = '0' then
            param_msg_error := get_error_msg_php('HR2102',global_v_lang,get_label_name('HRRCATE2',global_v_lang,'320'));
            json_str_output := get_response_message(null,param_msg_error,global_v_lang);
            return;          
        end if;
		...
		for i in 0..arr_params.get_size-1 loop
            ...
            if p_flgDelete then
                ...
                if sql%rowcount = 0 then
                        ...
                        insert into trcparam (codcompy, mailalno,dteeffec,numseq,fparam,ffield,descript,
        ...
        begin
			INSERT INTO trcalert (codcompy,mailalno,dteeffec,typemail,flgeffec,subject, message,syncond,codsend,qtydayb, qtydayr,statement,codcreate,coduser)
		...
		if arr_assign is not null then
			for i in 0..arr_assign.get_size-1 loop
                ...
                v_flg_assign        := hcm_util.get_string_t(obj_row, 'flg');
                v_seqno             := hcm_util.get_string_t(obj_row, 'seqno');
				if v_flg_assign = 'delete' then
					begin
						delete trcasign
						 where codcompy = p_codcompy
                           and mailalno = p_mailalno
					...
					begin
                        if v_flgappr = '3' then
                            v_flgsecu   := secur_main.secur7(v_codcompap, global_v_coduser);
                            if not v_flgsecu then
                                param_msg_error := get_error_msg_php('HR3007',global_v_lang);
                                json_str_output := get_response_message(null,param_msg_error,global_v_lang);
                                return;             
                            end if;
                        elsif v_flgappr = '4' then
                            v_flgsecu   := secur_main.secur2(v_codempap,global_v_coduser,global_v_zminlvl,global_v_zwrklvl,v_zupdsal);
                            if not v_flgsecu then
                                param_msg_error := get_error_msg_php('HR3007',global_v_lang);
                                json_str_output := get_response_message(null,param_msg_error,global_v_lang);
                                return;             
...
--
--                            if v_staemp = '9' then
--                                param_msg_error := get_error_msg_php('HR2101',global_v_lang,get_label_name('HRRCATE2',global_v_lang,'360'));
--                                json_str_output := get_response_message(null,param_msg_error,global_v_lang);
--                                return;   
--                            elsif v_staemp = '0' then
--                                param_msg_error := get_error_msg_php('HR2102',global_v_lang,get_label_name('HRRCATE2',global_v_lang,'360'));
--                                json_str_output := get_response_message(null,param_msg_error,global_v_lang);
--                                return;   
--                            end if;
                        end if;
                        update trcasign
                         ...
                         where codcompy = p_codcompy
                           ...
                           and seqno = v_seqno;
                         if sql%rowcount = 0 then
                                insert into trcasign (codcompy, mailalno, dteeffec, seqno,flgappr,codcompap,codposap,codempap,message,coduser,codcreate)
        ...
        if v_count_assign = 0 AND p_typemail != '10' then
            param_msg_error := get_error_msg_php('HR2045',global_v_lang,get_label_name('HRRCATE2',global_v_lang,'360'));
            json_str_output := get_response_message(null,param_msg_error,global_v_lang);
            rollback;
            return;        
        end if; 
...
--                            end if;   
                            v_numseq := nvl(v_numseq,0) + 1; 
                            insert into tmailrrc (codcompy, mailalno, dteeffec, pfield,
        ...
        select count(*)
         ...
         where codcompy = p_codcompy
           and mailalno = p_mailalno
           and dteeffec = p_dteeffec;        
        if v_count_report = 0 then
            param_msg_error := get_error_msg_php('HR2045',global_v_lang,get_label_name('HRRCATE2',global_v_lang,'370'));
            json_str_output := get_response_message(null,param_msg_error,global_v_lang);
            rollback;
            return;        
        end if;        
	...
	procedure genDetail (json_str_output out clob) is
        ...
        v_row			number := 0;
        v_desc			varchar2(200 char);
        v_message_display trcalert.message%type;
        v_dteeffec      date;
	...
	procedure post_delete_report_format (json_str_input in clob, json_str_output out clob) as
        ...
        p_mailalno          tmailrrc.mailalno%type;
        p_flgDelete         boolean;
        p_flgdesc           tmailrrc.flgdesc%type;
        p_numseq            tmailrrc.numseq%type;
        p_pfield            tmailrrc.pfield%type;
        p_pdesct            tmailrrc.pdesct%type;
	begin
		initial_value(json_str_input);
	...
	begin
		...
		if v_number_mail > 0 then
			case p_typemail
				when '10' then alertmsg_rc.gen_tappoinf(p_codcompy,'N',p_mailalno,p_dteeffec,'N');
				when '20' then alertmsg_rc.gen_empnotconf(p_codcompy,'N',p_mailalno,p_dteeffec,'N');
				when '30' then alertmsg_rc.gen_tbudget(p_codcompy,'N',p_mailalno,p_dteeffec,'N');
				when '40' then alertmsg_rc.gen_ttotguar_over(p_codcompy,'N',p_mailalno,p_dteeffec,'N');
				when '50' then alertmsg_rc.gen_tcolltrl(p_codcompy,'N',p_mailalno,p_dteeffec,'N');
				when '60' then alertmsg_rc.gen_resigncollateral(p_codcompy,'N',p_mailalno,p_dteeffec,'N');
				when '70' then alertmsg_rc.gen_guarantorretire(p_codcompy,'N',p_mailalno,p_dteeffec,'N');
				when '80' then alertmsg_rc.gen_welcomeemp(p_codcompy,'N',p_mailalno,p_dteeffec,'N');
				when '90' then alertmsg_rc.gen_jobpostingexp(p_codcompy,'N',p_mailalno,p_dteeffec,'N');
			end case;
