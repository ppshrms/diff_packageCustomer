  ...
  begin
      ...
      for r1 in c1 loop
        ...
        if (r1.codcomp = v_codcomp and r1.codpos = v_codpos) then
           obj_data.put('flgDisabledDetail','N'); --Y = ???? Tab ???? /N = ????????
        else
             obj_data.put('flgDisabledDetail','Y'); --Y = ???? Tab ???? /N = ????????
        end if;
         -->>user25 Date:30/09/2021  #4564
  ...
  begin
    ...
    for i in 0..obj_tab2.get_size-1 loop
      ...
      if v_flgDelete then
        ...
        begin -- softberry || 21/09/2023 || #9425   
        ...
        for r1 in c_com_pos loop
          ...
          if v_numpath is null or v_numpath <> r1.numpath then
            ...
            for i in c1 loop
              ...
              obj_data.put('flgDisabledCareer','Y'); --Y = ???? Tab ???? /N = ???????? --<<user25 Date:30/09/2021  #4564
              obj_row.put(to_char(v_row), obj_data);
              v_row        := v_row + 1;
              -- softberry || 2/10/2023 || #9366   || exit;
              end if;
            end loop;
            v_numpath := r1.numpath;
          end if;
...
--                                 and d.numpath = p_numpath) ;
    ...
    obj_datarows := json_object_t();
    v_row := 0;
    for i in c1 loop -- softberry || 2/10/2023 || #9366   || for i in 1..5 loop
      v_row := v_row + 1;
      obj_datatab2 := json_object_t();
--<< softberry || 2/10/2023 || #9366   
/*
      ...
      obj_datatab2.put('desc_codcomp', '?????????????????');
      obj_datatab2.put('desc_codlinef', '??? Quality Assurance (??? 0004)');
...
*/
      obj_datatab2.put('agepos', i.agepos);
      obj_datatab2.put('codcomp', i.codcomp);
      obj_datatab2.put('codlinef', i.codlinef);
      obj_datatab2.put('codpos', i.codpos);
      obj_datatab2.put('desc_codcomp', get_tcenter_name(i.codcomp,global_v_lang));
      obj_datatab2.put('desc_codlinef', get_tfunclin_name(hcm_util.get_codcomp_level(i.codcomp,1),i.codlinef,global_v_lang));
      obj_datatab2.put('desc_codpos', get_tpostn_name(i.codpos,global_v_lang));
        begin
          select dteefpos
            into v_dteefpos
          from temploy1
        ...
        obj_data.put('dteefpos',to_char(v_dteefpos, 'dd/mm/yyyy'));
        if v_dteefpos is not null then
          v_dteposdue := ADD_MONTHS( v_dteefpos, i.agepos );
        end if;
        obj_data.put('dteposdue',to_char(v_dteposdue, 'dd/mm/yyyy'));      
      obj_datatab2.put('numseq', v_row);   
-->> softberry || 2/10/2023 || #9366         
...
*/      
      cursor c1 is
        select distinct minnumseq, codlinef
          from ( select min(numseq) as minnumseq,codlinef
                from tposempd
                where codempid = p_codempid
                group by codlinef order by minnumseq ) subquery1
            order by minnumseq;
-->> softberry || 2/10/2023 || #9366   
 ...
 procedure gen_career_tab4_table1(json_str_output out clob)as
  ...
  procedure gen_career_tab4_table2(json_str_output out clob)as
    ...
    cursor c3 is
      select coddevp
        from tcomptdev    
--<< softberry || 20/09/2023 || #9424  
/*
       where codskill = v_codskill 
          and grade between (v_grade_emp+1) and v_grade  
*/
        where exists ( select 'x'
                        from tjobposskil b, tcmptncy c
                        where codcomp = v_codcomp
                        and codpos = v_codpos
                        and b.codskill = tcomptdev.codskill
                        and b.codskill = c.codtency
                        and c.codempid = p_codempid
                        and tcomptdev.grade between c.grade+1 and b.grade
                )
-->> softberry || 20/09/2023 || #9424          
      group by coddevp
      order by coddevp;
...
--    end loop; -- softberry || 20/09/2023 || #9424 
