...
-- last update: 15/09/2020 14:00
  ...
  begin
    ...
    for i in c1 loop
          ...
          obj_data.put('numprlvl',i.numlevel); --????????
          if i.numlevel = 0 then
            obj_data.put('desc_codcompp',get_tcenter_name(i.codcompy,global_v_lang)); --???????????????  ???????? = 0
          else
          ...
          obj_data.put('desc_codcomps',''); --??????????????????????
          obj_data.put('desc_codposs','');  --?????????????????????
          obj_data.put('qty',''); --?????
    ...
    ----?????????????????
          ...
          for k in c3 loop
            ...
            obj_data.put('desc_codcomps',get_tcenter_name(k.codcompp,global_v_lang)); --??????????????????????
            obj_data.put('desc_codposs',get_tpostn_name(k.codpos,global_v_lang));  --?????????????????????
            obj_data.put('codpos',k.codpos);  --?????????????????????
...
--msg_err2('IN gen_data###2 v_rcnt  =  '||v_rcnt);
              ...
              obj_data.put('desc_codempid',get_temploy_name(i.codresp,global_v_lang));--????-???????
              obj_data.put('desc_codcomps',get_tcenter_name(k.codcompp,global_v_lang)); --??????????????????????
              obj_data.put('desc_codposs',get_tpostn_name(k.codpospr,global_v_lang));  --?????????????????????
              --obj_data.put('qty',k.qtyexman); --?????
              obj_row.put(to_char(v_rcnt-1),obj_data);
              v_rcnt2 := v_rcnt2 + 1;
          end loop;*/
          -->>User37 #7481 1. RP Module 16/01/2022
