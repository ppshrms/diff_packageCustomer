...
--<< softberry || 19/09/2023 || #9416   
         and exists ( select  'x'
               from   tbudget
              where   dteyrbug                = b_index_year
                and   codcomp                 = b.codcompp
                and   nvl(codpos,'####')      = nvl(b.codpospr,nvl(codpos,'####'))
...
-->> softberry || 19/09/2023 || #9416   
    ...
    cursor c3 is
      select nvl(qtyreqyr,0)qtyreqyr ,
             nvl(qtypromote,0) qtypromote,
             nvl(qtyreqyr,0)+nvl(qtypromote,0) total,
             amttotbudgt, codappr, dteappr ,staappr,dtereq,codemprq,codcomp,codpos
        from tbudget
       where codcomp  like b_index_codcomp||'%'
         and dtereq   = nvl(b_index_dtereq,dtereq)
         and codappr  = nvl(b_index_codappr,codappr)
         and nvl(staappr,'#')        = nvl(b_index_staappr,nvl(staappr,'#'))
       order by dtereq asc;
    --<< issue4448#10143 28/11/2023
 begin
    ...
    if b_index_codcomp = '' or b_index_codcomp is null then
      ...
      for j in c3 loop
        ...
        obj_data.put('codpos',j.codpos);
        obj_data.put('desc_codpos',get_tpostn_name(j.codpos,global_v_lang));
        obj_data.put('addp',j.qtyreqyr);
        obj_data.put('promp',j.qtypromote);
        obj_data.put('totalp',j.total);
        obj_data.put('budget',to_char(j.amttotbudgt,'fm999,999,990.00'));
        obj_data.put('approvby',get_temploy_name(j.codappr,global_v_lang));
        obj_data.put('dteappr',to_char(j.dteappr,'dd/mm/yyyy'));
        obj_data.put('statusappr',get_tlistval_name('STAAPPR',j.staappr,global_v_lang));
