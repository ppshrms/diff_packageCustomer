...
--------------------------------------------------------
  ...
  begin
    ...
    p_posflag               := hcm_util.get_string_t(json_obj,'p_posflag'); -- softberry || 18/09/2023 || #9428

    p_codcompe          := hcm_util.get_string_t(json_obj,'p_codcompe');
    p_codpose           := hcm_util.get_string_t(json_obj,'p_codpose');
    -- softberry || 19/09/2023 || #9428
    if p_codpose = 'ALL' then
        p_codpose := '';
    end if;
    if p_posflag <> 'ALL' OR p_posflag is null then -- softberry || 19/09/2023 || #9428
        p_dteeffec          := to_date(hcm_util.get_string_t(json_obj,'p_dteeffec') ,'ddmmyyyy');
    end if; -- softberry || 19/09/2023 || #9428
...
--<< softberry || 18/09/2023 || #9428
    cursor c1_all is
      --Baby Boomer--
      select a.codempid, a.codcompe, a.codpose, a.jobgrade, b.dteempmt, b.dteempdb ,b.qtywkday ,b.dteeffex, a.dteeffec, a.codcomp
      from  ttalente a, temploy1 b
      where a.codempid   = b.codempid
        and a.codcompe   like  p_codcompe|| '%'
        and a.codpose    = nvl(p_codpose, a.codpose)
        and dteeffec   = (select max(dteeffec)
                            from ttalente b
                           where b.codcompe = a.codcompe
                             and b.codpose  = a.codpose)
...
-->> softberry || 18/09/2023 || #9428
  begin
    obj_row := json_object_t();
    if p_posflag <> 'ALL' OR p_posflag is null then -- softberry || 18/09/2023 || #9428
        for r1 in c1 loop
          v_flgdata := 'Y';
...
--<< softberry || 18/09/2023 || #9428
    elsif p_posflag = 'ALL' then
        for r1 in c1_all loop
          v_flgdata := 'Y';
          v_secur := secur_main.secur2(r1.codempid,global_v_coduser,global_v_zminlvl,global_v_zwrklvl,v_zupdsal);
          if v_secur then
            v_flgsecur := 'Y';
