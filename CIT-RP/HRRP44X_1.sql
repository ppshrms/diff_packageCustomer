    ...
    cursor c1 is
      select codcomp ,codpos ,qtybudgt
       ...
       where dteyrbug  = b_index_year
         and dtemthbug = decode(b_index_year,to_char(sysdate,'yyyy'),to_number(to_char(sysdate,'mm')),12)   --????????????????? ??????????????????? ???????????????????? ??? Fix ????? = 12
...
--  --  --  --  --  --  --  --  --  --  --  --  --  --  --
 ...
 procedure check_index is
  ...
  begin
    ...
    v_item31 := get_label_name('HRRP44XC2', global_v_lang, '70');   -- ????????
        ...
        if v_secur then
          ...
          for n in 1..5 loop
                  ...
                  if n = 1 then
                     ...
                     v_item8  := get_label_name('HRRP44XC2', global_v_lang, '60'); --???????????
                     -- col.1  Data in table
                     v_item10 := i.qtybudgt;
                  end if;
                  ...
                  ----------Axis X detail(from data row)
                     ...
                     v_item8  := get_label_name('HRRP44XC2', global_v_lang, '50'); --???????????????
                     -- col.2  Data in table
                     v_item10 := v_present;
                  end if;
                  ...
                  ----------Axis X detail(from data row)
                     v_item7  := '03'; --'moveout';
                     v_item8  := get_label_name('HRRP44XC2', global_v_lang, '40'); --???????????
                     -- col.3  Data in table
                  ...
                  ----------Axis X detail(from data row)
                     v_item7  := '04'; --'during';
                     v_item8  := get_label_name('HRRP44XC2', global_v_lang, '30'); --??????????????
                     -- col.4  Data in table
                     v_item10 := v_procur;
                  end if;
                  --
                  ...
                  ----------Axis X detail(from data row)
                       v_item7  := '05'; --'rate';
                       v_item8  := get_label_name('HRRP44XC2', global_v_lang, '20'); --?????????
                       -- col.5  Data in table
                  ...
                  v_item6  := get_label_name('HRRP44XC2', global_v_lang, '80'); --???????

                  ----------Axis Y Label
                  v_item9  := get_label_name('HRRP44XC2', global_v_lang, '10'); --????????????
