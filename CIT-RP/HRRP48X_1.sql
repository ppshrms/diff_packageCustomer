...
-- last update: 19/08/2020 11:00
  ...
  begin
    ...
    b_index_typerep    := hcm_util.get_string_t(json_obj,'p_typerep');  --????????????? (1-????????????? , 2-???????????????)
  ...
  begin
    initial_value(json_str_input);
    if param_msg_error is null then
      if b_index_typerep = '1' then   -- softberry || 6/11/2023 || #10131     
        gen_popup(json_str_output);
--<< softberry || 6/11/2023 || #10131         
      else  
...
--<< softberry || 6/11/2023 || #10131  
  procedure gen_popup2(json_str_output out clob) as
    ...
    cursor c1 is 
      select dteoccup,qtydatrq, dteretire, codempid, codcomp, codpos, dteempdb, dteempmt  -- softberry || 21/11/2023 || #10131 || select dteretire, codempid, codcomp, codpos, dteempdb, dteempmt
        from temploy1
       where codcomp like b_index_codcomp||'%'
         and codpos   =   b_index_codpos
         and to_char(add_months(dteoccup,qtydatrq),'YYYY') = b_index_dteyear
  ...
  begin
    ...
    for i in c1 loop
      ...
      if flgpass then
          ...
          obj_data.put('coderror', '200');
          obj_data.put('seq',v_rcnt);
          obj_data.put('dteretire',to_char(add_months(i.dteoccup,i.qtydatrq),'dd/mm/yyyy')); -- softberry || 21/11/2023 || #10131 || obj_data.put('dteretire',to_char(i.dteretire,'dd/mm/yyyy'));
          obj_data.put('image', get_emp_img(i.codempid));
          obj_data.put('codempid',i.codempid);
          obj_data.put('codempid_desc',get_temploy_name(i.codempid,global_v_lang));
          obj_data.put('pos_desc',get_tpostn_name(i.codpos,global_v_lang));
...
-->> softberry || 6/11/2023 || #10131    
  ...
  begin
    ...
    v_item31 := get_label_name('HRRP48X2', global_v_lang, '140'); --'?????????????????????????';
...
--    v_item1  := b_index_typerep; --get_label_name('HRRP48XC2', global_v_lang, '40');
    ...
    for i in c1 loop
        ...
        for v_month in 1..12 loop
            ...
            ----------??? Y
                ...
                v_item9  := get_label_name('HRRP48X2', global_v_lang, '150'); --'????????????'; 
            ----------?????????
  ...
  begin
    ...
    exception when others then
     ...
     v_item31 := get_label_name('HRRP48X2', global_v_lang, '160');-- softberry || 27/11/2023 || #10130 || v_item31 := get_label_name('HRRP48X2', global_v_lang, '140'); -- softberry || 3/11/2023 || #10130 || v_item31 := get_label_name('HRRP48XC2', global_v_lang, '210'); --'?????????????????????????';
--    v_item14 := b_index_typerep;
--    v_item1  := b_index_typerep; --get_label_name('HRRP48XC2', global_v_lang, '40');
    ...
    for i in c1 loop
        ...
        for v_month in 1..12 loop
            ----------??? X
                ...
                v_item6  := b_index_dteyear; -- softberry || 3/11/2023 || #10130 || v_item6  := get_label_name('HRRP48XC2', global_v_lang, '140'); --'????????????'; 
            ----------??? Y
                v_item7  := lpad(v_month, 2, '0');
                v_item8  := get_tlistval_name('NAMMTHFUL', v_month, global_v_lang);
                v_item9  := get_label_name('HRRP48X2', global_v_lang, '150');-- softberry || 3/11/2023 || #10130 || v_item9  := b_index_dteyear;
            ----------?????????
                if v_month    = 1 then v_item10  := i.qtymth1;
                elsif v_month = 2 then v_item10  := i.qtymth2;
                elsif v_month = 3 then v_item10  := i.qtymth3;
