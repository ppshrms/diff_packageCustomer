...
--------------------------------------------------------
  ...
  procedure process_save_index(json_str_input in clob,json_str_output out clob) is
    ...
    v_codcomp_tsucctrn      tsucctrn.codcomp%type; -- softberry || 4/10/2023 || #9347  
    ...
    v_codcomp_tsuccdev      tsuccdev.codcomp%type; -- softberry || 4/10/2023 || #9347   
...
--      param_detail_row
      ...
      if v_flg = 'add' or v_flg = 'edit' then
         ...
         for j in 0..(param_detail1.get_size - 1) loop
            ...
            v_codcomp_tsucctrn   := hcm_util.get_string_t(param_detail1_row,'codcomp'); -- softberry || 4/10/2023 || #9347   
            ...
            begin
                if v_flgAdd and not v_flgDelete then -- softberry || 4/10/2023 || #9347 || if v_flgAdd then
                    ...
                    delete tsucctrn
                     where codempid = v_codempid
                       and codcomp = v_codcomp_tsucctrn -- softberry || 4/10/2023 || #9347and codcomp = b_index_codcomp
                       and codpos = b_index_codpos
                       and codcours = v_codcours;
                end if;
            exception when others then
                param_msg_error := dbms_utility.format_error_stack||' '||dbms_utility.format_error_backtrace;
                rollback;
                -- return; -->> softberry || 21/09/2023 || #9431   || return;
            end;
         end loop;
         ...
         for k in 0..(param_detail2.get_size - 1) loop    
            ...
            v_codcomp_tsuccdev  := hcm_util.get_string_t(param_detail2_row,'codcomp'); -- softberry || 4/10/2023 || #9347   
            ...
            begin
                if v_flgAdd and not v_flgDelete then -- softberry || 4/10/2023 || #9347 || if v_flgAdd then
                    ...
                    delete tsuccdev
                     where codempid = v_codempid
                       and codcomp = v_codcomp_tsuccdev -- softberry || 4/10/2023 || #9347 || and codcomp = b_index_codcomp
                       and codpos = b_index_codpos
                       and coddevp = v_coddevp;
                end if;
            exception when others then
  ...
  begin
    ...
    for r1 in c_tjobposskil loop
        ...
        for r2 in c_tcomptcr loop
            begin
                select dtestr,dteend,dtetrst,codcomp -- softberry || 4/10/2023 || #9347  || select dtestr,dteend,dtetrst
                  into v_dtestr,v_dteend,v_dtetrst,v_codcomp_tsucctrn -- softberry || 4/10/2023 || #9347  || into v_dtestr,v_dteend,v_dtetrst
                  from tsucctrn
                 where codempid = p_codempid_query
    ...
    for r3 in c_tab1 loop
      ...
      obj_data.put('codcomp', v_codcomp_tsucctrn); -- softberry || 4/10/2023 || #9347 
  ...
  procedure gen_detail_develop(json_output out json_object_t) is
    ...
    v_destarget         tsuccdev.destarget%type;
    v_desresults        tsuccdev.desresults%type;
    v_codcomp           tsuccdev.codcomp%type; -- softberry || 4/10/2023 || #9347
  ...
  begin
    ...
    for r1 in c_tjobposskil loop
        ...
        for r2 in c_tcomptdev loop
            begin
                select destarget,dtestr,dteend,desresults,codcomp -- softberry || 4/10/2023 || #9347  || select destarget,dtestr,dteend,desresults 
                  into v_destarget,v_dtestr,v_dteend,v_desresults,v_codcomp -- softberry || 4/10/2023 || #9347  || into v_destarget,v_dtestr,v_dteend,v_desresults
                  from tsuccdev
                 where codempid = p_codempid_query
                   and codcomp like b_index_codcomp||'%'
                   and codpos = b_index_codpos
    ...
    for r3 in c_tab2 loop
      ...
      obj_data.put('coderror','200');
      obj_data.put('numseq',v_rcnt + 1);
      obj_data.put('codcomp',v_codcomp); -- softberry || 4/10/2023 || #9347
