...
-- last update: 11/08/2020 14:00
  ...
  begin
    ...
    b_index_stasuccr   := hcm_util.get_string_t(json_obj,'p_stasuccr');  --?????
  ...
  --
  procedure check_index is --user36 #3082 09/09/2021
    v_cnt   number := 0;
  begin    
    begin
         ...
         where a.codcomp  like b_index_codcomp||'%'
           ...
           and 0 <> (select count(ts.codcomp)
                      ...
                      where ts.coduser = global_v_coduser
                        and a.codcompe like ts.codcomp || '%'
                        and rownum <= 1);
      end;
      if v_cnt = 0 then
  ...
  begin
    ...
    check_index; --user36 #3082 09/09/2021
