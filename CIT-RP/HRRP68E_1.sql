...
--------------------------------------------------------
  ...
  procedure gen_detail_data(json_str_output out clob)as
    ...
    v_message       varchar2(4000 char); -- softberry || 19/10/2023 || #9368  
    cursor c1 is
  ...
  begin
    ...
    --<< softberry || 19/10/2023 || #9368  
    if v_staappr = 'C' then
        v_message := replace(get_error_msg_php('HR8014',global_v_lang),'@#$%400','');
    end if;
    -->> softberry || 19/10/2023 || #9368  
    for r1 in c1 loop
      v_flgsecu := secur_main.secur2(r1.codempid, global_v_coduser, global_v_zminlvl, global_v_zwrklvl, global_v_zupdsal, global_v_numlvlsalst, global_v_numlvlsalen);
      if v_flgsecu then
        if r1.staappr <> 'P' then
            v_staappr := r1.staappr;
            --<< softberry || 19/10/2023 || #9368  
            if v_staappr = 'A' then
                v_message := replace(get_error_msg_php('HR8011',global_v_lang),'@#$%400','');
            end if;
            -->> softberry || 19/10/2023 || #9368  
            exit;
        end if;
    ...
    --<< softberry || 19/10/2023 || #9368  
    obj_data.put('message', replace(v_message,'@#$%400',''));
    -->> softberry || 19/10/2023 || #9368  
    ...
    if v_stmt is not null then
      begin
        ...
        while dbms_sql.fetch_rows(v_cursor_id) > 0 loop
          ...
          v_flgsecur   := secur_main.secur2(v_codempid,global_v_coduser,global_v_zminlvl,global_v_zwrklvl,global_v_zupdsal, global_v_numlvlsalst, global_v_numlvlsalen);
          if v_flgsecur then
              begin
  ...
  begin
    initial_value(json_str_input);
    -- softberry || 8/11/2023 || #10124 || check_save;
    if param_msg_error is null then
      ...
      for i in 0..params_json.get_size-1 loop
        ...
        if v_flg = 'add' then
          b_index_codselect   := v_codempid;  -- softberry || 8/11/2023 || #10124  
          check_save;  -- softberry || 8/11/2023 || #10124  
          begin
