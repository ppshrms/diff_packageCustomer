...
--------------------------------------------------------
  ...
  begin
    ...
    --box
    for r1 in c1 loop
      get_data_box(b_index_year, b_index_codcomp, hcm_util.get_codcomp_level(b_index_codcomp,1), r1.codgroup, v_amountemp, v_percntemp);
      get_name_box(r1.codgroup, hcm_util.get_codcomp_level(b_index_codcomp,1), v_dtechoose, v_namgroupt, v_descgroup); ----
      obj_box := json_object_t();
      obj_box.put('codgroup', r1.codgroup);
      obj_box.put('namgroupt', v_namgroupt);
      obj_box.put('descgroup', v_descgroup);
  ...
  begin
    begin
      select nvl(descgroup,''), nvl(namgroupt,'')--, nvl(statement,''), nvl(syncond ,'')
        into p_descgroup, p_namgroupt--, v_statement, v_syncond
        from tninebox
       where codcompy = p_codcompy
         and dteeffec = p_dteeffec
         and codgroup = p_codgroup;
    exception when no_data_found then
      p_namgroupt := '';    p_descgroup := '';
