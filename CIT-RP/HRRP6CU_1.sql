...
--------------------------------------------------------
  ...
  begin
    ...
    for r1 in c1 loop
      ...
      v_flgAppr   := chk_flowmail.check_approve('HRRP6CU',r1.codempid,v_approvno,global_v_codempid,'','',p_check);
      if v_flgAppr then
        v_rcnt      := v_rcnt+1;
        obj_data    := json_object_t();
...
--    v_staappr     ttalente.staappr%type;
  ...
  begin
    ...
    for i in 0..params_json.get_size-1 loop
      ...
      v_flgAppr   := chk_flowmail.check_approve('HRRP6CU',v_codempid,v_approvno,p_codappr,'','',p_check);
...
--          insert_ttemprpt(global_v_coduser,'HRRP6CU'||'REPLY', v_codempid,  null ,null,null,null);
      ...
      begin
          v_error_cc := chk_flowmail.send_mail_reply('HRRP6CU', v_codempid, null , global_v_codempid, global_v_coduser, null, 'HRRP6CU3', 70, 'U', v_flgStaappr, v_approvno, null, null, 'TNINEBOXE', v_rowid, '1', null);
      EXCEPTION WHEN OTHERS THEN
      ...
      if v_flgAppr AND p_check = 'N' AND v_flgStaappr <> 'N' then
        begin
            v_error := chk_flowmail.send_mail_for_approve('HRRP6AB', v_codempid, global_v_codempid, global_v_coduser, null, 'HRRP6CU3', 70, 'E', 'P', v_approvno, null, null,'TNINEBOXE',v_rowid, '1', null);
        EXCEPTION WHEN OTHERS THEN
            v_error_sendmail := '2403';
        END;
      else
    ...
    if param_msg_error is null then
        IF v_error_sendmail <> '2403' OR v_error_sendmail is null THEN
