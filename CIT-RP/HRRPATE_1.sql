--------------------------------------------------------
--  DDL for Package Body HRRPATE
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "HRRPATE" is
-- last update: 25/05/2018 12:23
  procedure initial_value(json_str in clob) is
	...
	begin
		...
		for i in 0..arr_destroy.get_size-1 loop
			...
			begin
				delete from trpalert where codcompy = p_codcompy and mailalno = p_mailalno and dteeffec = p_dteeffec;
				delete from trpparam where codcompy = p_codcompy and mailalno = p_mailalno and dteeffec = p_dteeffec;
				delete from trpasign where codcompy = p_codcompy and mailalno = p_mailalno and dteeffec = p_dteeffec;
				delete from tmailrrp where codcompy = p_codcompy and mailalno = p_mailalno and dteeffec = p_dteeffec;
			end;
		end loop;
  ...
  function get_seqno (v_codcompy varchar2, v_mailalno varchar2, v_dteeffec date) return number is
	...
	procedure saveData (json_str_input in clob, json_str_output out clob) as obj_row json_object_t;
        ...
        p_codsend               trpalert.codsend%type;
        p_code                  trpalert.syncond%type;
        p_statement             trpalert.statement%type;
		v_rcnt			        number := 0;
		...
		p_ffield		        trpparam.ffield%type;
        p_descript              trpparam.descript%type;
		p_codtable		        trpparam.codtable%type;
		p_fparam		        trpparam.fparam%type;
		p_flgdesc	            trpparam.flgdesc%type;
        p_flgdesc_b             boolean;
		p_numseq		        trpparam.numseq%type;
		v_numseq		        number := 0;
		p_flgDelete		        boolean;

        p_pfield                tmailrrp.pfield%type;
        p_pdesct                tmailrrp.pdesct%type;
        v_flgappr               trpasign.flgappr%type;
        v_codcompap             trpasign.codcompap%type;
        v_codposap              trpasign.codposap%type;
        v_codempap              trpasign.codempap%type;
        v_message               trpasign.message%type;
...
--            param_msg_error := get_error_msg_php('HR3007',global_v_lang,get_label_name('HRRPATE2',global_v_lang,'360'));
            param_msg_error := get_error_msg_php('HR3007',global_v_lang);
            json_str_output := get_response_message(null,param_msg_error,global_v_lang);
            return;
        end if;
            ...
            if p_flgDelete then
                ...
                begin
                    ...
                    if sql%rowcount = 0 then
                        ...
                        insert into trpparam (codcompy, mailalno,dteeffec,numseq,fparam,ffield,descript,
        ...
        begin
			INSERT INTO trpalert (codcompy,mailalno,dteeffec,typemail,flgeffec,subject, message,syncond,codsend,qtydayb, qtydayr,statement,codcreate,coduser)
		...
		if arr_assign is not null then
					...
					begin
						...
						update trpasign
						 ...
						 where codcompy = p_codcompy
                           ...
                           and seqno = v_seqno;
						if sql%rowcount = 0 then
							insert into trpasign (codcompy, mailalno, dteeffec, seqno,flgappr,codcompap,codposap,codempap,message,coduser,codcreate)
            ...
            merge into trpasign upd
                using (
                  select rowid, row_number() over (order by seqno) rnum
                    from trpasign
                   where codcompy = p_codcompy
                     and mailalno = p_mailalno
                     and dteeffec = p_dteeffec
            ...
            on (upd.rowid = s.rowid)
            when matched then update set upd.seqno = s.rnum;
		end if;
        ...
        if v_count_assign = 0 then
            param_msg_error := get_error_msg_php('HR2045',global_v_lang,get_label_name('HRRPATE2',global_v_lang,'360'));
            json_str_output := get_response_message(null,param_msg_error,global_v_lang);
            rollback;
            return;
        end if;
				...
				if p_flgDelete then
					...
					begin
						...
						if sql%rowcount = 0 then
                            ...
                            insert into tmailrrp (codcompy, mailalno, dteeffec, pfield,
        ...
        if v_count_report = 0 then
            param_msg_error := get_error_msg_php('HR2045',global_v_lang,get_label_name('HRRPATE2',global_v_lang,'370'));
            json_str_output := get_response_message(null,param_msg_error,global_v_lang);
            rollback;
            return;
        end if;
	...
	procedure genDetail (json_str_output out clob) is
        ...
        v_row			number := 0;
        v_desc			varchar2(200 char);
        v_message_display trpalert.message%type;
        v_dteeffec      date;
	...
	procedure post_delete_report_format (json_str_input in clob, json_str_output out clob) as
        ...
        p_mailalno          tmailrrp.mailalno%type;
        p_flgDelete         boolean;
        p_flgdesc           tmailrrp.flgdesc%type;
        p_numseq            tmailrrp.numseq%type;
        p_pfield            tmailrrp.pfield%type;
        p_pdesct            tmailrrp.pdesct%type;
	begin
		initial_value(json_str_input);
	...
	begin
		...
		if v_number_mail > 0 then
			case p_typemail
				when '10' then alertmsg_rp.gen_retire(p_codcompy,'N',p_mailalno,p_dteeffec,'N');
				when '20' then alertmsg_rp.gen_endwork(p_codcompy,'N',p_mailalno,p_dteeffec,'N');
                when '30' then alertmsg_rp.gen_tposempd(p_codcompy,'N',p_mailalno,p_dteeffec,'N');
                when '40' then alertmsg_rp.gen_resign(p_codcompy,'N',p_mailalno,p_dteeffec,'N');
			end case;
