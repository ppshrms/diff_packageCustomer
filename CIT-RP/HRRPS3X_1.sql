...
--    p_logic3       := hcm_util.get_json_t(json_obj,'p_syncond3');
    ...
    global_v_codempid   := hcm_util.get_string_t(json_obj,'p_codempid'); -- softberry || 18/09/2023 || #9418
  end initial_value;
  --
...
--<< softberry || 18/09/2023 || #9418   
    procedure  insert_graph (json_str_output in json_object_t) as
     ...
     p_row               json_object_t;
     p_index_head_col      json_object_t;
     p_row_head_col      json_object_t;
     p_index_child_col      json_object_t;
     p_row_child_col      json_object_t;
     v_count              number := 0;
     v_col_desc_head      varchar2(200 char);
     v_col_desc_child      varchar2(200 char);
     v_cal              number := 0;
     ...
     begin
        begin
          delete
            from ttemprpt
           where codapp = 'HRRPS3X'
             and codempid = global_v_codempid;
        end;   
         graph_y_desc    := get_label_name('HRRPS3X1',global_v_lang,140);

         p_table  := hcm_util.get_json_t(json_str_output,'table');
         p_row  := hcm_util.get_json_t(p_table,'rows');
         p_index_head_col := hcm_util.get_json_t(json_str_output,'indexHeadParentCol');
         p_index_child_col := hcm_util.get_json_t(json_str_output,'indexHeadChildCol');
        for i in 1..p_row.get_size loop 
            v_row_json      := hcm_util.get_json_t(p_row,i-1);
            v_grade           := hcm_util.get_string_t(v_row_json, 'description');
            v_qtyin          := hcm_util.get_string_t(v_row_json, 'qtyin');
            v_qtyres           := hcm_util.get_string_t(v_row_json, 'qtyres');     
            for j in 1..p_index_head_col.get_size loop
                p_row_head_col  := hcm_util.get_json_t(p_index_head_col,j-1);
                v_qtyin := hcm_util.get_string_t(v_row_json, 'data'||j);
                v_col_desc_head := hcm_util.get_string_t(p_row_head_col, 'col'||j);
                v_count := 0;
                for k in 1..p_index_child_col.get_size loop    
                    v_count := v_count+1;
                    p_row_head_col  := hcm_util.get_json_t(p_index_child_col,k-1);
                    v_cal := (p_index_child_col.get_size*(j-1))+k;
                    v_qtyin := hcm_util.get_string_t(v_row_json, 'data'||v_cal);
                    v_col_desc_child := hcm_util.get_string_t(p_row_head_col, 'col'||k);
                    ...
                    VALUES (global_v_codempid, 'HRRPS3X', v_seq,
                    '','','',
                    v_grade,
                    v_grade,graph_y_desc,
                    v_qtyin, v_col_desc_child,
                    graph_y_desc,
                    '',null,v_col_desc_head);
                    v_seq := v_seq + 1;
               end loop;                   
                if v_count <= 0 then 
                    INSERT INTO TTEMPRPT (CODEMPID,CODAPP,NUMSEQ,
...
-->> softberry || 18/09/2023 || #9418    
  ...
  procedure gen_data(json_str_output out clob) is
    ...
    v_cntsub        number;
    v_row           number;
    v_data_no      number; -- softberry || 20/11/2023 || #10137  
  ...
  begin
	...
	if b_index_codcompy is not null and b_index_codwork is not null then
        v_statment := 'select codempid,codcomp'||
                      '  from v_hrrps3x where   staemp in (''1'',''3'')  and codcomp in ('||
                              'select b.codcompp
                    ...
                    for j in 1..v_rcnt loop
                        ...
                        v_num := execute_sql('select count(codempid) 
                                                ...
                                                where codcomp like '''||b_index_codcomp||'%'''||
                                                 ' and staemp in (''1'',''3'') '||
                                                 ' and '||b_index_splitrow||' = '''||v_splitrow||''' 
                                                  and '||b_index_splitcol||' = '''||a_splitcol(j)||'''
                                                  and '||a_code(i));
                        v_data_no :=  ((j-1)*v_cntsub)+i; -- softberry || 27/11/2023 || #10137 || v_data_no :=  ((j-1)*3)+i;   -- softberry || 20/11/2023                           
                        obj_data.put('data'||v_data_no, v_num );-- softberry || 20/11/2023 || #10137 -- softberry || 15/11/2023 || #10135  || obj_data.put('data'||v_row, v_num);
                    end loop;
                end loop;
            else
    ...
    if v_flgdata = 'N' then
      ...
      insert_graph(obj_rowmain); -- softberry || 18/09/2023 || #9418
