...
--------------------------------------------------------
  ...
  begin
    begin
       select round(avg(b.qtyexman)) into v_qtyexman --????? ??????? ??????
         from tmanpwm b
  ...
  begin
    begin
      select a.dteyrbug, sum(a.qtytotrc) qtytotrc, sum(a.qtytotrf) qtytotrf
       ...
       group by dteyrbug
       order by dteyrbug;
      v_sum := v_qtytotrc + v_qtytotrf;  --???????????????????????? + ????????????????????????--<< user25 date: 09/09/2021 #4603 #4606


----#3531       v_sum := v_qtytotrc + v_qtytotrf;  --???????????????????????? + ????????????????????????
--       v_sum := v_qtytotrc;  --????????????????????????
----#3531
  ...
  begin
    begin
     select nvl(round(avg(b.qtyexman)),0) into v_qtyexman --????? ??????? ??????
       from tmanpwm b
      where hcm_util.get_codcomp_level(b.codcomp,1) = b_index_codcompy
        and b.dteyrbug = v_year
        and exists (select codcomp
  ...
  begin
    begin
      select a.dteyrbug, sum(a.qtytotre) qtytotre, sum(a.qtytotro) qtytotro
       ...
       group by dteyrbug
       order by dteyrbug;
       v_sum := v_qtytotre + v_qtytotro;  --??????????????? + ????????????????--<< user25 date: 09/09/2021 #4603 #4606


----#3531       v_sum := v_qtytotre + v_qtytotro;  --??????????????? + ????????????????
--       v_sum := v_qtytotre;  --???????????????
----#3531
    ...
    for i in 1..v_arrayCount loop
      ...
      if v_year is not null then
        ...
        for r1 in c1 loop
          ...
          --
     v_qtynew := r1.qtytotrc + r1.qtytotrf; --???????????????????????? + ???????????????????????? --<< user25 date: 09/09/2021 #4603 #4606

----#3531          v_qtynew := r1.qtytotrc + r1.qtytotrf; --???????????????????????? + ????????????????????????
--          v_qtynew := r1.qtytotrc; --????????????????????????
----#3531
          ...
          --
       v_qtyout := r1.qtytotre + r1.qtytotro;  --??????????????? + ???????????????? --<< user25 date: 09/09/2021 #4603 #4606


----#3531          v_qtyout := r1.qtytotre + r1.qtytotro;  --??????????????? + ????????????????
--          v_qtyout := r1.qtytotre;  --???????????????
----#3531
