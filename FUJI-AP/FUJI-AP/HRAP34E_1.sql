...
--------------------------------------------------------
  ...
  begin
    ...
    begin
      ...
      --v_scoreta    := to_number(hcm_util.get_string_t(obj_worktime_detail,'score')); ---#9859
      v_scoreta    := to_number(hcm_util.get_string_t(obj_worktime_detail,'netscoreta')); ---#9859
      v_scorfta    := to_number(hcm_util.get_string_t(obj_worktime_detail,'scorfta'));
      --v_scorepunsh := to_number(hcm_util.get_string_t(obj_worktime_detail,'scorepunsh')); ---#9859
      v_scorepunsh := to_number(hcm_util.get_string_t(obj_worktime_detail,'netscorepunsh')); ---#9859
      v_scorfpunsh := to_number(hcm_util.get_string_t(obj_worktime_detail,'scorfpunsh'));

      p_qtyta      := v_scoreta ;---#9859
      p_qtypuns    := v_scorepunsh;---#9859
...
--v_flg_chk_disable := false; -- disable record
    ...
    cursor c_tappfm is
      select *
       ...
       where dteyreap  = b_index_dteyreap
         and numtime   = b_index_numtime
         -- and codaplvl  = b_index_codaplvl -- bk surachai | 20230310 | #9993
        ...
        begin
          insert into tappfm (codempid,dteyreap,numtime,numseq,
              ...
              values (v_codempid,b_index_dteyreap,b_index_numtime,v_numseq,
                      v_jobgrade,b_index_codapman,b_index_dteapman,v_global_dteapstr,v_global_dteapend,
