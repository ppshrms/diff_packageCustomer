...
-- last update: 07/08/2020 09:40
  ...
  begin
    ...
    for r1 in c1 loop
      ...
      v_flgpass     := chk_flowmail.check_approve('HRAP37U', r1.codempid, v_approvno, global_v_codempid, null, null, v_check);
      if nvl(r1.codappr,'#$%@') <> global_v_codempid then
          if (v_flgpass) then
              v_secur       := 'Y';
              obj_data      := json_object_t();
              v_rcnt        := v_rcnt + 1;
              ...
              if r1.pctadjsal is null then
                ...
                obj_data.put('pctadjsal',to_char(v_pctadjsal,'fm990.00'));
              else
                obj_data.put('pctadjsal',to_char(r1.pctadjsal,'fm990.00'));
              end if;
              --obj_data.put('pctadjsal',to_char(nvl(r1.pctadjsal,r1.pctcalsal),'fm990.00'));
              ...
              obj_data.put('pctnet',r1.pctcalsal);  

              --<< softberry || 15/08/2023 || #9793 
              obj_data.put('codadj',r1.codadj);
              if r1.codadj is not null then
                obj_data.put('codadj_asterisk', '*'); 
              else
                obj_data.put('codadj_asterisk', ''); 
              end if;
              -->> softberry || 15/08/2023 || #9793 
  ...
  begin
    ...
    for i in 0..p_index_rows.get_size-1 loop
        ...
        v_pctnet        := to_number(hcm_util.get_string_t(v_row,'pctnet'));

        v_rcnt_main     := v_rcnt_main + 1;
        v_flgpass       := chk_flowmail.check_approve('HRAP37U', v_codempid, v_approvno, global_v_codempid, null, null, v_check);
        obj_row         := json_object_t();
        for r1 in c1 loop
            v_rcnt          := v_rcnt +1;
            obj_data        := json_object_t();
  ...
  procedure send_approve(json_str_input in clob, json_str_output out clob) as
    ...
    v_jobgradet     temploy1.jobgrade%type;
    v_codgrpgl      temploy1.codgrpgl%type;
    v_codgrpglt     temploy1.codgrpgl%type;
    v_amtincom1     number;
    v_amtincadj1    number;
    v_amtincom2     number;
    v_amtincom3     number;
    ...
    v_amtincadje    temploy3.amtincom1%type;
...
--    check_save;
    if param_msg_error is null then
        begin
            for i in 0..p_selected_rows.get_size-1 loop
                ...
                v_pctnet        := hcm_util.get_string_t(obj_row,'pctnet');

                v_amtincadj1    := nvl(to_number(v_amtbudg) + to_number(v_amtadj),0);
                ...
                if v_staappr = 'N' then
                    ...
                    begin                       
                        v_error := chk_flowmail.send_mail_reply('HRAP37U', v_codempid, global_v_codempid, global_v_codempid,global_v_coduser, null, 'HRAP37UP1', 99, 'U', v_staappr, v_approvno + 1, null, null,'TAPPRAIS',v_rowid, '1', null);                      
                    exception when others then
                    ...
                    if v_check = 'Y' then
                        ...
                        begin
                            insert into ttmovemt (codempid,dteeffec,numseq,codtrn,codcomp,
                                                ...
                                                jobgrade,jobgradet,codgrpgl,codgrpglt,
                            ...
                            values (v_codempid,v_dteeffec,v_numseq,v_codtrn,v_codcomp,
                                    ...
                                    v_jobgrade,v_jobgrade,v_codgrpgl,v_codgrpgl,
                        ...
                        select codcomp, stddec(amtsal,v_codempid, global_v_chken)
                        ...
                        begin
                        v_pctnet := 0;
                        if v_amtincadj1 <> 0 then
                            v_pctnet := round( (( nvl(v_amtincadj1,0) - nvl(tapprais_amtsal,0) *100) / tapprais_amtsal) ,2);
                        end if;
                            ...
                            insert into ttranpm (dteyreap,codempid,codcomp,codtrn,dteeffec,
                                         ...
                                         values (v_dteyreap,v_codempid,tapprais_codcomp,v_codtrn,v_dteeffec,
                                                 stdenc(nvl(tapprais_amtsal,0),v_codempid, global_v_chken),stdenc(nvl(v_amtincadj1,0), v_codempid, global_v_chken),
                                                 stdenc(nvl(v_amtsaln,0), v_codempid, global_v_chken),v_pctnet,
                        ...
                        exception when dup_val_on_index then
                            update ttranpm
                               set codcomp  = tapprais_codcomp,
                                   codtrn   = v_codtrn,
                                   dteeffec = v_dteeffec,
                                   amtsal   = stdenc(nvl(tapprais_amtsal,0),v_codempid, global_v_chken),
                                   amtadj   = stdenc(nvl(v_amtincadj1,0), v_codempid, global_v_chken),
                                   amtsaln  = stdenc(nvl(v_amtsaln,0), v_codempid, global_v_chken),
                                   pctnet   = v_pctnet,
                                   codappr  = v_codappr,
                                   dteappr  = v_dteappr,
                                   remarkap = v_remark,
                        ...
                        begin
                            ...
                            chk_flowmail.replace_text_frmmail(v_templete_to, 'TBONUS', v_rowid, get_label_name('HRAP37UP1', global_v_lang, 99), v_codform, '1', null, global_v_coduser, global_v_lang, v_msg_to);
                            v_error := chk_flowmail.send_mail_to_emp (v_codempid, global_v_coduser, v_msg_to, NULL, get_label_name('HRAP37UP1', global_v_lang, 99), 'U', global_v_lang, null);
                        exception when others then
                            param_msg_error_mail := get_error_msg_php('HR2403',global_v_lang);
                        end;*/
                    else
                        ...
                        select rowid
                         ...
                         where dteyreap = v_dteyreap
                           and codempid = v_codempid;
                        begin
                            v_error := chk_flowmail.send_mail_for_approve('HRAP37U', v_codempid, global_v_codempid, global_v_coduser, null, 'HRAP37UP1', 99, 'U', v_staappr, v_approvno + 1, null, null,'TAPPRAIS',v_rowid, '1', null);
                        exception when others then
                          param_msg_error_mail := get_error_msg_php('HR2403',global_v_lang);
                        end;
                    end if;
  ...
  begin  null;
    ...
    if v_pctcalsal is null and v_amtbudg is null then
        v_err_code := 'HR2045';
        v_err_text := v_err_code||' '||get_errorm_name(v_err_code,global_v_lang);
        return;
    end if;
  ...
  begin
    ...
    for i in 0..param_json.get_size-1 loop
      ...
      if v_err_text is null then
          ...
          begin
            select typpayroll, jobgrade, dteempmt,numlvl
              into v_typpayroll, v_jobgrade, v_dteempmt,v_numlvl
              from temploy1
             where codempid = v_codempid;
          exception when no_data_found then
            param_msg_error := dbms_utility.format_error_stack||' '||dbms_utility.format_error_backtrace;
          ...
          v_amtbudgn := 0;
          v_amtsaln  := 0;
          if nvl(v_pctcalsal,0) <> 0 then
            v_amtbudgn := (( nvl(v_amtincom1,0) * v_pctcalsal)/100) ;
            v_amtbudg  := v_amtbudgn;
          else
              if nvl(v_amtbudg,0) <> 0 then
                v_pctcalsaln := round( (( (nvl(v_amtincom1,0) + nvl(v_amtbudg,0)) - nvl(v_amtincom1,0) *100) / v_amtincom1) ,2);
                v_pctcalsal  := v_pctcalsaln;
              end if;
          end if;

          v_amtsaln := v_amtincom1 + v_amtbudg + v_amtadj;
          ...
          begin
            insert into tapprais (codempid,dteyreap,codcomp,codpos,
                          ...
                          values (v_codempid,p_dteyreap,v_codcomp,v_codpos,
                                  ...
                                  v_grade,v_qtyscore,v_pctcalsal,stdenc(v_midpoint, v_codempid, global_v_chken),
                                  stdenc(v_amtincom1, v_codempid, global_v_chken),stdenc(v_amtbudg, v_codempid, global_v_chken),stdenc(v_amtadj, v_codempid, global_v_chken),stdenc(v_amtsaln, v_codempid, global_v_chken),
                                  stdenc(v_amtmaxsa, v_codempid, global_v_chken),stdenc(v_amtminsa, v_codempid, global_v_chken),v_remark,'N',
                                  sysdate,global_v_coduser,sysdate,global_v_coduser,
                                  v_numlvl,'P');

            begin
                insert into tlogapprais (codempid,dteyreap,dteedit,
                                         amtsal,pctsal,amtbudg,
                                         amtadj,amtsaln,grade,
                                         qtyscore,pctcalsal,dtecreate,
                                         codcreate,dteupd,coduser)
                               values   (v_codempid,p_dteyreap,v_dteedit, 
                                         stdenc(v_amtincom1, v_codempid, global_v_chken),v_pctcalsal,stdenc(v_amtbudg, v_codempid, global_v_chken),
                                         stdenc(v_amtadj, v_codempid, global_v_chken),stdenc(v_amtsaln, v_codempid, global_v_chken),v_grade, 
                                         v_qtyscore, v_pctcalsal,sysdate, 
          ...
          exception when dup_val_on_index then
            begin
                update tapprais set grade       = v_grade,
                                    qtyscore    = v_qtyscore,
                                    pctcalsal   = v_pctcalsal,
                                    amtsal      = stdenc(v_amtincom1, v_codempid, global_v_chken),
                                    amtbudg     = stdenc(v_amtbudg, v_codempid, global_v_chken),
                                    amtadj      = stdenc(v_amtadj,  v_codempid, global_v_chken),
                                    amtsaln     = stdenc(v_amtsaln, v_codempid, global_v_chken),
                                    coduser     = global_v_coduser
            ...
            if (v_pctcalsal is not null and v_pctcalsal > 0)
                or (v_amtbudg is not null and v_amtbudg > 0)
                or (v_amtadj is not null and v_amtadj > 0) then            
                begin
                    insert into tlogapprais (codempid,dteyreap,dteedit,
  ...
  procedure cal_adjsalary(json_str_input in clob, json_str_output out clob) as
    ...
    v_amtsaln       number;
    v_pctnet        number;
		is_amtbudg			varchar(5 char);
		is_pctadjsal		varchar(5 char);
  ...
  begin
    ...
    p_codcomp       := hcm_util.get_string_t(obj_dataRow,'codcomp');
		p_codempid       := hcm_util.get_string_t(obj_dataRow,'codempid');
    v_amtmidsal     := hcm_util.get_string_t(obj_dataRow,'amtmidsal');
		v_amtbudg				:= hcm_util.get_string_t(obj_dataRow, 'amtbudg');
    ...
    v_amtadj        := hcm_util.get_string_t(obj_dataRow,'amtadj');
		...
		is_amtbudg 			:= hcm_util.get_string_t(json_obj, 'is_amtbudg');
		is_pctadjsal 		:= hcm_util.get_string_t(json_obj, 'is_pctadjsal');
    ...
    if param_msg_error is null then
        begin
					if is_amtbudg = 'true' then
						v_pctadjsal := round((v_amtbudg / greatest(v_amtsal, 0)) * 100, 2);
					end if;

					if is_pctadjsal = 'true' then
							...
							v_amtbudg       := round(v_amtbudg);
					end if;

						v_amtsaln       := nvl(v_amtsal, 0) + nvl(v_amtbudg, 0) + nvl(v_amtadj, 0);

            if nvl(v_amtsal,0) <> 0 then
                v_pctnet        := round(((nvl(v_amtbudg,0) + nvl(v_amtadj,0)) * 100) / nvl(v_amtsal,0),2);
            end if;
