...
procedure get_detail(json_str_input in clob, json_str_output out clob) as
    ...
    cursor c_tapprais is
      select codcomp,numlvl,dteyreap,codempid,jobgrade,grade,pctcalsal,pctadjsal, -- softberry || 4/07/2023 || #8872 || select codempid,jobgrade,grade,pctcalsal,pctadjsal,
             ...
             stddec(amtadj,codempid,v_chken) amtadj, -- softberry || 4/07/2023 || #8872
             stddec(amtceiling,codempid,v_chken) amtceiling, -- softberry || 4/07/2023 || #8872
             stddec(amtover,codempid,v_chken) amtover,
             stddec(amtbudg,codempid,v_chken) amtbudg
        from tapprais
  ...
  begin
    ...
    for i in c_tapprais loop
        ...
        if flgpass then
            ...
            obj_data.put('grade_desc', get_tstdis_name (i.codcomp, i.dteyreap , i.grade,global_v_lang)); -- softberry || 4/07/2023 || #8872 || obj_data.put('grade_desc', get_tlistval_name ('INSTGRD1' ,i.grade, global_v_lang));
            ...
            if v_zupdsal = 'Y' then
                ...
                obj_data.put('amtbudg', nvl(i.amtbudg,0));
                obj_data.put('amtsaln', nvl(i.amtsaln,0));
                obj_data.put('amtover', nvl(i.amtover,0));
                v_belowmin := 0;

                if to_number(nvl(i.amtsaln,0)) < to_number(nvl(i.amtmidsal,0)) then--520,000.50 - 68,000.00
                    v_belowmin := to_number(nvl(i.amtmidsal,0))- to_number(nvl(i.amtsaln,0));--ex  min 30000 sal 29000 >> 1000
                end if;
                obj_data.put('amtbelmn', nvl(v_belowmin,0));---nvl(i.amtminsal,0) - nvl(i.amtsaln,0));
                --obj_data.put('amtbudg', nvl('i.amtadj',0)); -- softberry || 4/07/2023 || #8872
                if to_number(nvl(i.amtsaln,0)) > to_number(nvl(i.amtceiling,0)) then
                    obj_data.put('amtbudgoverceil', to_number(nvl(i.amtsaln,0)) - to_number(nvl(i.amtceiling,0)));
                else
                    obj_data.put('amtbudgoverceil', '');
                end if;-- softberry || 4/07/2023 || #8872*/
            else
                obj_data.put('amtmidsal','');
                obj_data.put('pctupsal', '');
                obj_data.put('amtbudg', '');
                obj_data.put('amtsaln', '');
                obj_data.put('amtover', '');
                obj_data.put('amtbelmn','');
                obj_data.put('amtbudgoverceil', '');
            end if;
