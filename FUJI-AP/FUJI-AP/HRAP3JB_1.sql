...
--------------------------------------------------------
  ...
  procedure gen_index (json_str_output out clob) is
    ...
    v_rec2          number := 0; -- softberry || 11/07/2023 || #9536
    cursor c_tapppy is
...
-->> softberry || 11/07/2023 || #9536
        obj_data.put('numrec', v_rec2); -- softberry || 11/07/2023 || #9536 ||   obj_data.put('numrec', '');
  ...
  procedure check_transfer_data is
    ...
    cursor c_tdtepay is
        select numperiod,dtemthpay,dteyrepay
          from tdtepay
         where codcompy   = b_index_codcompy -- softberry || 03/08/2023 || #8739  where codcompy   = p_codcompy
...
--        p_codcomp       := hcm_util.get_string_t(param_json_row,'codcomp'); -- softberry || 03/08/2023 || #9789
...
--        p_codcompy      := hcm_util.get_string_t(param_json_row,'codcompy'); -- softberry || 03/08/2023 || #9789
        check_transfer_data;
        if param_msg_error is null then
            transfer_data;
            v_rcnt       := v_rcnt+1;
