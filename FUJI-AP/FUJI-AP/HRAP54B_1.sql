...
begin
    ...
    for i in c_tbonparh loop
        v_data := 'Y';
        exit;
    end loop;

    if v_data = 'N' then
        param_msg_error := get_error_msg_php('HR2010', global_v_lang, 'TBONPARH');
        return;
    end if;
...
procedure process_data(json_str_output out clob) is
    ...
    v_typbon        varchar2(10 char) ;

    cursor c_ttbonus is
        select dteyreap,numtime,codbon,codempid,codcomp,
               codpos,jobgrade,numcond,
...
begin
    ...
    begin
        select grdyear,grdnumtime,typbon
          into v_grdyear,v_grdnumtime,v_typbon
          from ttbonparh
         where dteyreap = p_dteyreap
           and numtime  = p_numtime
    ...
    cursor c_tbonus is
      select codempid,grade,numcond,codpos,jobgrade,dteempmt,qtybon,pctdedbo,qtydaybon,
       ...
       where dteyreap  = p_dteyreap
         ...
         and codbon    = p_codbon
         and codcomp like p_codcomp||'%'
      order by grade,numcond,codcomp,codempid,codpos;
