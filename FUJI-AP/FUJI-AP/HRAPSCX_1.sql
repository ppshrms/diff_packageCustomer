...
-- last update: 02/09/2020 21:00
  ...
  begin
    ...
    b_index_compbudg    := get_compbudg(b_index_codcomp,b_index_dteyreap); 
    ...
    begin
      select count(codempid)
       ...
       where dteyreap  = b_index_dteyreap
         ...
         and get_compbudg(codcomp,b_index_dteyreap) = b_index_compbudg;
    end;
    -----
    begin
      select count(codempid)
...
state_query     varchar2(5000 char);

  cursor c1 is
    select a.grade,nvl(a.pctemp,0) pctemp
      from tstdis a
     where a.codcomp  = b_index_compbudg 
  ...
  cursor c_comp is 
    select a.codcomp 
      from tstdis a
     where b_index_codcomp||'%' like codcomp||'%' ---Edit 14/02/2023 Pratya    
       and a.dteyreap = (select max(b.dteyreap)
                           from tstdis b
                          where b.codcomp   = a.codcomp
                            and b.dteyreap <= b_index_dteyreap)
      ...
      for i in c1 loop
        ...
        if flgpass then
          ...
          -- 2-กำหนดคะแนนและ %พนักงาน
            ...
            obj_data.put('policy',round(cs_emp_poli,2));
          end if;
...
--    v_numseq    ttemprpt.numseq%type := 0;
  ...
  cursor c1 is
     select a.grade,nvl(a.pctemp,0) pctemp
       from tstdis a
      where a.codcomp   = b_index_compbudg ---- like b_index_codcomp||'%'
        and a.dteyreap  = (select max(b.dteyreap)
                            from tstdis b
                           where b.codcomp = a.codcomp
                             and b.dteyreap <= b_index_dteyreap)
  ...
  cursor c_comp is ----
    select a.codcomp 
      from tstdis a
     where b_index_codcomp||'%' like codcomp||'%'     
       and a.dteyreap = (select max(b.dteyreap)
                           from tstdis b
                          where b.codcomp   = a.codcomp
                            and b.dteyreap <= b_index_dteyreap)
    ...
    if v_flgdata = 'Y' then
      ...
      begin
       select a.flggrade
         into v_flggrade
         from tapbudgt a
        where a.codcomp   = b_index_compbudg ----= b_index_codcomp
          and a.dteyreap  = (select max(b.dteyreap)
                               from tapbudgt  b
                              where b.codcomp   = a.codcomp
                                and b.dteyreap <= b_index_dteyreap);
...
--================================================================
          begin
            select  count(codempid)
             ...
             where  dteyreap    = b_index_dteyreap
               and  numtime     = b_index_numtime
               and  get_compbudg(codcomp,b_index_dteyreap) = b_index_compbudg ----||codcomp     like b_index_codcomp||'%'
               and  nvl(flgappr,'P') = 'C'
               and  grdap       = i.grade;
          end;
          -- cs_emp_grd := 10; --<< Apisit || 03/08/2023 || issue #9796 >>--
          ----
          begin
            select  count(codempid)
              into  cs_emp_tot
...
--======================================================
  ...
  function get_compbudg(p_codcomp in varchar2,p_dteyreap in number) return varchar2 is ----
    v_codcompbudg     tstdis.codcomp%type;

  cursor c_comp is ----
    select a.codcomp 
      from tstdis a
     where p_codcomp||'%'  like codcomp||'%'---codcomp||'%' like p_codcomp||'%'    
       and a.dteyreap = (select max(b.dteyreap)
                           from tstdis b
                          where p_codcomp||'%'  like codcomp||'%'---codcomp||'%' like p_codcomp||'%'   
  ...
  begin
    for i in c_comp loop
      v_codcompbudg := i.codcomp;
      exit;
    end loop;
    return v_codcompbudg;
  end;
  --
end;
