--------------------------------------------------------
--  DDL for Package Body HRAPSJ1
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "HRAPSJ1" is
...
--      json(json_str_output).print;
  ...
  begin
--msg_err2('IN gen_head ##1 b_index_codcomp  =  '||b_index_codcomp);
--msg_err2('IN gen_head ##1 b_index_codpos   =  '||b_index_codpos);
--HEAD REPORT 2 ROW
    obj_row_codskill := json_object_t();
    for i in c1 loop
          v_codskill  := i.codskill;
          v_rcnt2     := 0;
msg_err2('IN gen_head ##1  v_codskill  =  '||v_codskill);
    ----
          obj_row_codcours := json_object_t();
          for k in c2 loop
msg_err2('IN gen_head ##2  k.codcours  =  '||k.codcours);
