...
--------------------------------------------------------
  ...
  begin
    ...
    b_index_pflag           := to_number(hcm_util.get_string_t(json_obj,'p_flag'));
  ...
  begin
    ...
    if v_flggrade = '3' then
        for i in c1 loop
          ...
          obj_data.put('desgrad', i.desgrade);
          obj_data.put('desgrade', i.desgrade);
          obj_data.put('desgradt', i.desgrade);
          obj_data.put('desgrad3', i.desgrade);
          obj_data.put('desgrad4', i.desgrade);
          obj_data.put('desgrad5', i.desgrade);
          obj_data.put('pctemp', '');
        ...
        if v_flggrade = '2' then    --< Apisit || 20/07/2023 || #9583
            for i in c2 loop
              v_rcnt := v_rcnt+1;
              obj_data := json_object_t();
              obj_data.put('coderror', '200');
...
-- << softberry || 11/08/2023 || #9469
      update tapbudgt
         set flggrade =	v_flggrade,
             ...
             dteupd = sysdate,
             coduser = global_v_coduser
       where codcomp like b_index_codcomp||'%' /*get_compful(b_index_codcomp)*/
         and dteyreap = b_index_dteyreap;
-- >> softberry || 11/08/2023 || #9469
    if v_isCopy = 'Y' then
      begin
...
--        end if;
  ...
  --
  --< Apisit || 20/07/2023 || #9583
  procedure gen_tindex(json_str_output out clob) is
    obj_row         json_object_t;
    obj_data        json_object_t;
    obj_result      json_object_t;
    ...
    cursor c1 is
      select *
        from tgradekpi a
       where codcompy = hcm_util.get_codcomp_level(b_index_codcomp,1)
         and dteyreap = (select max(b.dteyreap)
                        from tgradekpi  b
                        where b.codcompy = a.codcompy
                            and  b.dteyreap  <= b_index_dteyreap)
       order by grade,desgrade desc;
...
--        if secur_main.secur7(i.codcomp, global_v_coduser) then
            ...
            obj_data.put('coderror', '200');
              ...
              obj_data.put('flggrade', TO_CHAR(b_index_pflag));
              obj_data.put('grade', i.grade);
              obj_data.put('desgrad', i.desgrade);
              obj_data.put('desgrade', i.desgrade);
              obj_data.put('desgradt', i.desgrade);
...
--        end if;
  ...
  -- Apisit || 20/07/2023 || #9583
  procedure get_tindex(json_str_input in clob, json_str_output out clob) as
  begin
    initial_value(json_str_input);
    if param_msg_error is null then
      gen_tindex(json_str_output);
    else
      json_str_output := get_response_message(null,param_msg_error,global_v_lang);
    end if;
  exception when others then
