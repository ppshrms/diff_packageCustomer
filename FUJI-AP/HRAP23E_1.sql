...
--      if v_secur then
      if v_secur and v_zupdsal = 'Y' then
        ...
        if nvl(v_amt_before_adj,0) > 0 then
          v_adj_percent   := round(((v_amt_after_adj - v_amt_before_adj) *100) / v_amt_before_adj,2); --#9827
        end if;

        if v_amtadjin <> 0 then
  ...
  procedure save_adj(json_str_input in clob,json_str_output out clob) is
    ...
    param_json_table      json_object_t; -- softberry || 11/08/2023 || #9828
  ...
  begin
    ...
    param_json_table      := hcm_util.get_json_t(json_input,'table'); -- softberry || 11/08/2023 || #9828
...
*/
    for i in 0..(param_json_table.get_size - 1) loop
      json_table_row        := hcm_util.get_json_t(param_json_table,to_char(i));
      t_ttemadj2.numtime    := t_ttemadj1.numtime;
      t_ttemadj2.codincom   := hcm_util.get_string_t(json_table_row,'codincom');
      t_ttemadj2.codempid   := hcm_util.get_string_t(json_table_row,'codempid');
      v_amtincod            := replace(hcm_util.get_string_t(json_table_row,'amtincod'),',','');
...
-->> softberry || 11/08/2023 || #9828
    ...
    t_ttemadj1.numemp     := param_json_table.get_size; -- softberry || 11/08/2023 || #9828  ||  t_ttemadj1.numemp     := json_tables.get_size;
