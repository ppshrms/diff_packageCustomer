...
--Redmine #5552
  ...
  procedure gen_detail(json_str_output out clob) is
    ...
    v_dteapstr_default  tstdisd.dteapstr%type; --Redmine #9992
    v_dteapend_default  tstdisd.dteapend%type; --Redmine #9992
  ...
  begin
    ...
    if v_response is not null then
      ...
      begin
          select flgapman, flgappr, dteapstr, dteapend, dteapman, hcm_util.get_codcomp_level(codcomp,1),codcomp,codpos
            into v_flgapman, v_flgappr, v_dteapstr, v_dteapend, v_dteapman, v_codcompy, v_codcomp, v_codpos
            from tappfm
           where codempid = p_codempid_query
             and dteyreap = p_dteyreap
             and numtime = p_numtime
...
--Redmine #5552
      ...
      begin
          select flgtypap,dteapstr,dteapend --Redmine #9992
            into v_flgtypap,v_dteapstr_default,v_dteapend_default   --Redmine #9992
            from tstdisd
...
--Redmine #9992
      v_dteapstr := nvl(v_dteapstr,v_dteapstr_default);
      v_dteapend := nvl(v_dteapend,v_dteapend_default);
--Redmine #9992
...
--Redmine #5552
    ...
    if (nvl(v_pctta,0) + nvl(v_pctpunsh,0) > 0) then
        ...
        if v_qtyta is null and v_qtypuns is null then
          ...
          update tappemp set qtyta   = v_scoreta,
                             qtypuns = v_scorepunsh
         where codempid = p_codempid_query
           and dteyreap = p_dteyreap
           and numtime = p_numtime;
        else
          v_qtyta              := round((v_pctta * v_qtyta)/(v_pctta * v_scorfta) * 100,2);
          v_qtypuns            := round((v_pctpunsh * v_qtypuns) / (v_pctpunsh * v_scorfpunsh) * 100,2);           
        end if;
...
--Redmine #5552
      ...
      if v_tappemp_qtyta is not null or v_tappemp_qtypuns is not null then
          ...
          v_rcnt    := 0;
          v_scoreta := v_scorfta;
          v_tappemp_qtyta     := v_scoreta;--Issue #10108 27/11/2023
          obj_grpleave_row := json_object_t();
          for r_tattpre1 in c_tattpre1 loop
            v_rcnt          := v_rcnt + 1;
            obj_data        := json_object_t();
          ...
          v_rcnt            := 0;
          v_scorepunsh      := v_scorfpunsh;
          v_tappemp_qtypuns := v_scorepunsh; --Issue #10108 27/11/2023
          obj_punnish_row   := json_object_t();
...
--        obj_data.put('qtyscorn3',v_qtyscorn3);
  ...
  procedure gen_competencysub(json_str_output out clob) is
    ...
    cursor c_tcomptcr is
        select distinct codcours -- softberry || 04/08/2023 || #9486 || select *
          from tcomptcr
...
--        v_qtyta := round((v_scoreta/v_scorfta*100),2);
        --v_qtyta := round((v_scoreta/v_scorfta),2);
    --Redmine #9992
    --Issue #10108 27/11/2023
        v_qtyta :=  v_scoreta ;
    end if;
...
--        v_qtypuns := round((v_scorepunsh/v_scorfpunsh*100),2);
        --v_qtypuns := round((v_scorepunsh/v_scorfpunsh),2);
     --Redmine #9992
     --Issue #10108 27/11/2023
        v_qtypuns := v_scorepunsh ;
    end if;
...
--    p_numseq                := hcm_util.get_string_t(json_input,'p_numseq');
  ...
  begin
       ...
       and numtime = p_numtime;
        ...
        begin
            select scorfta, scorfpunsh
              ...
              from tattpreh
             where codcompy = hcm_util.get_codcomp_level(v_taplvl_codcomp,1)
               and codaplvl = p_codaplvl
               and dteeffec = (select max(dteeffec)
                                 from tattpreh
                                where codcompy = hcm_util.get_codcomp_level(v_taplvl_codcomp,1)
    ...
    v_scrta     := (((nvl(t_tappemp.qtyta,0) * nvl(v_scorfta,0)) * nvl(t_taplvl.pctta,0)) /100) ;
    v_scrpuns   := (((nvl(t_tappemp.qtypuns,0) * nvl(v_scorfpunsh,0)) * nvl(t_taplvl.pctpunsh,0)) /100);
    --End Issue #10108 27/11/2023
