...
--------------------------------------------------------
  ...
  procedure gen_index(json_str_output out clob) is
    ...
    v_rcnt      number := 0;
    v_count_tk  number := 0; -- softberry || 10/07/2023 || #9525
    cursor c1 is 
  ...
  begin
    ...
    for i in c1 loop
      ...
      --<< softberry || 10/07/2023 || #9525
      begin
        select count('x') into v_count_tk
          from tkpiemp
      ...
      if v_count_tk = 0 then
        obj_data.put('can_delete', true); 
      else
        obj_data.put('can_delete', false); 
      end if;
      -->> softberry || 10/07/2023 || #9525
...
--    check_index;
  ...
  procedure gen_object_kpi(json_str_output out clob) is
    ...
    v_count_tk  number := 0; -- softberry || 15/08/2023 || #9851
    ...
    cursor c_kpi is
      ...
      select 'D' as typkpi,codkpino as codkpi,'' as kpides,target,
       ...
       where dteyreap   = b_index_dteyreap
         and numtime    = b_index_numtime
         and codempid   = b_index_codempid
         -- softberry || 07/08/2023 || #9523 || and codcomp    like b_index_codcomp||'%'
  ...
  begin
    ...
    for i in c_kpi loop
      ...
      if v_count_tk = 0 then
        obj_data_kpi.put('can_delete', true); 
      else
        obj_data_kpi.put('can_delete', false); 
      end if;
      -->> softberry || 15/08/2023 || #9851      
...
--#3705
      ...
      values (b_index_dteyreap,b_index_numtime,b_index_codempid,t_tkpiemppl.codkpi,lpad(t_tkpiemppl.planno,4,'0'), -- softberry || 07/08/2023 || #9492 || values (b_index_dteyreap,b_index_numtime,b_index_codempid,t_tkpiemppl.codkpi,t_tkpiemppl.planno,
    ...
    if v_flg_copy then
      ...
      begin
        ...
        select b_index_dteyreap,b_index_numtime,b_index_codempid,codkpi,
               ...
               pctwgt,targtstr,targtend,grade,
               qtyscor,qtyscorn,achieve,mtrrn,
               v_codcomp,v_codpos,global_v_coduser,global_v_coduser
          from tkpiemp
      ...
      begin
        insert into tkpiemppl(dteyreap,numtime,codempid,
                              ...
                              codcreate,coduser)
        select b_index_dteyreap,b_index_numtime,b_index_codempid,
               codkpi,lpad(planno,4,'0'),plandes,targtstr, -- softberry || 07/08/2023 || #9492 || codkpi,planno,plandes,targtstr,
               targtend,dtewstr,dtewend,workdesc,
               global_v_coduser,global_v_coduser
          from tkpiemppl
         where codempid   = v_codempid_copy
...
--#3705
    ...
    if json_action_plan.get_size > 0 then
      for i in 0..(json_action_plan.get_size - 1) loop
        ...
        t_tkpiemppl.planno    := lpad(hcm_util.get_string_t(json_act_plan_row,'planno'),4,'0'); -- softberry || 07/08/2023 || #9492 || t_tkpiemppl.planno    := hcm_util.get_string_t(json_act_plan_row,'planno');
...
-->> user20 Date: 25/08/2021  AP Module- #5832
    ...
    begin
      ...
      values (b_index_dteyreap,b_index_numtime,p_col_value(1),v_kpino,p_col_value(4),
              ...
              to_date(p_col_value(10),'dd/mm/yyyy'),v_codcomp,v_codpos,global_v_coduser,global_v_coduser);
    exception when dup_val_on_index then
      update tkpiemp
...
-->> user20 Date: 25/08/2021  AP Module- #5832
    if p_col_value(14) is not null then
      begin
        insert into tkpiemppl(dteyreap,numtime,codempid,codkpi,planno,
                             plandes,targtstr,targtend,codcreate,coduser)
        values (b_index_dteyreap,b_index_numtime,p_col_value(1),v_codkpi,lpad(p_col_value(14),4,'0'), -- softberry || 07/08/2023 || #9492 || values (b_index_dteyreap,b_index_numtime,p_col_value(1),v_codkpi,p_col_value(14),
      ...
      if v_error_msg is null then
        begin
          ...
          values (b_index_dteyreap,b_index_numtime,arr_col_value('codempid'),arr_col_value('codkpi'),
                  ...
                  v_codcomp,v_codpos,'IMPORT','IMPORT');
        exception when dup_val_on_index then
          update tkpiemp
...
--    obj_data_sheet    := hcm_util.get_json_t(param_data,'dataSheet');
    ...
    for i in 0..(obj_tkpiemppl.get_size - 1) loop
      ...
      arr_col_value('planno')     := lpad(hcm_util.get_string_t(obj_tkpiemppl_row,'planno'),4,'0'); -- softberry || 07/08/2023 || #9492 || arr_col_value('planno')     := hcm_util.get_string_t(obj_tkpiemppl_row,'planno');
