...
begin
    for r_tappemp in c_tappemp loop
        ...
        v_remarkcal := replace(v_remarkcal,'@#$%400','');
                    ...
                    for j in 1..v_numsyn loop
                        ...
                        for r_tstdis in c_tstdis_grade loop
                            if v_arrgrdap(j) = r_tstdis.grade then
                                ...
                                if i.qtytotnet >= r_tstdis.pctwkstr then
                                    if v_empgrd < v_arrnumemp(j) then
                                        v_grdap     := r_tstdis.grade;
                                        exit;
                                    elsif v_empgrd >= v_arrnumemp(j) then
                                        begin
                                            select min(qtytotnet) into v_minqtytotnet
                                              from tappemp
                                             where dteyreap = p_dteyreap
                                               and numtime  = p_numtime
                                               and codcomp  like v_codcomp||'%'
                                        ...
                                        if v_minqtytotnet <> i.qtytotnet then
                                            v_grdap := null;
                                        else
                                            v_grdap := r_tstdis.grade;
                                            exit;
