...
--------------------------------------------------------
    ...
    for i in 1..9 loop
        ...
        if i = 1 then
            ...
            if v_codempid is not null then
                begin
                    select codempid,staemp,codcomp into v_codempid,v_staemp,v_codcomp
                      from temploy1
  ...
  begin
    ...
    for i in 0..param_import_row.get_size-1 loop
      ...
      gen_workingtime_detail(v_qtypuns,v_qtyta,v_pctdbon,v_pctdsal,v_flgsal,v_flgbonus,v_dteyreap,v_numtime); --- add parameter v_dteyreap,v_numtime ##9854
  ...
  procedure gen_workingtime_detail(p_qtypuns out number, p_qtyta out number, p_pctdbon out number,
                                   p_pctdsal out number, p_flgsal out varchar2, p_flgbonus out varchar2,
                                   p_dteyreap in number,p_numtime in number) is
  ...
  --  insert into a(a,b) values(v_pctta,to_char(v_global_dteapend,'dd/mm/yyyy')) ; commit;
    ----------------------------------------------------------------
