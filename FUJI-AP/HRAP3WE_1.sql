...
--------------------------------------------------------
  ...
  begin
    ...
    p_flggrade          := hcm_util.get_string_t(json_obj,'p_flggrade');    -- < apisit || 25/07/2023 || 4448 (#9722) >
...
--                obj_data.put('score4',r1.qtykpie3);
                ...
                obj_data.put('description',v_desgrad);
...
--                end if;
        ...
        chkflggrade := v_flggrade;  -- < apisit || 25/07/2023 || 4448 (#9722)
        ...
        if v_flggrade = '2' then
          ...
          obj_result.put('chkflggrade', chkflggrade);   -- < apisit || 25/07/2023 || 4448 (#9722)
...
--      order by codcomp desc ;
    ...
    -- < apisit || 25/07/2023 || 4448 (#9722)
        if p_flggrade <> 3 then
          begin
    ...
    --          and i_qtyadjtot between pctwkstr and pctwkend;
          exception when no_data_found then
           ...
           begin
              ...
              and grade = i_grdadj
              and dteyreap = tstdis_dteyreap
              and i_qtyadjtot between pctpostr and pctpoend;
            exception when no_data_found then
                param_msg_error := get_error_msg_php('HR2010',global_v_lang,'tstdis');
                return;
            end;
    ...
    if p_flggrade <> '3' then
        ...
        begin
            select grade
             ...
             where codcomp  = tstdis_codcomp
               and dteyreap = tstdis_dteyreap
                 and p_score between pctpostr and pctpoend;
        exception when no_data_found then
            v_grade := null;
        end;
    end if;
