...
--------------------------------------------------------
  ...
  begin
      ...
      -- < Apisit || 07/08/2023 || issue #9785
      p_codappr           := hcm_util.get_string_t(json_obj,'p_codempap');
...
--      end if;
  ...
  begin
    ...
    for i in c_tstdis loop
        ...
        -- < Apisit || 07/08/2023 || issue #9785
        obj_data.put('p_codappr', i.coduser);
        obj_data.put('p_dteappr', to_char(i.dteupd,'dd/mm/yyyy'));
        -- >
        v_numemp := 0;
        begin
            select count(codempid),
                   nvl(sum(stddec(amtbudg,codempid,v_chken)),0),
                   nvl(sum(stddec(amtover,codempid,v_chken)),0)
             into v_numemp,v_amtbudg,v_amtover
             from tappraism
    ...
    if param_msg_error is not null then
      ...
      if v_rcnt = 0 then
        ...
        obj_result.put('codempap',v_coduser);
...
--      insert_data_parallel (p_codapp,global_v_coduser,v_numproc)  ;
      for i in 0..p_param_json.get_size-1 loop
            ...
            v_percenOld      := hcm_util.get_string_t(param_json_row,'percenOld');
             -- < Apisit || 07/08/2023 || issue #9785
--            p_codappr        := hcm_util.get_string_t(param_json_row,'p_codappr');
--            p_dteappr        := to_char(p_dteappr,'dd/mm/yyyy');
            -- >

            update tstdis set pctpostr = v_percst, pctpoend = v_percen, coduser  = global_v_coduser
             where codcomp  = p_codcomp
               and dteyreap = p_dteyreap
               and grade    = v_grade ;
            -- < Apisit || 07/08/2023 || issue #9785
            /*
            if p_codappr is not null and p_dteappr is not null then

                update tappraism set codappr = p_codappr, dteappr = p_dteappr, coduser  = global_v_coduser
                 where codcomp  like p_codcomp||'%'
                   and dteyreap = p_dteyreap;

                update tstdis set pctactstr = v_percst, pctactend = v_percen, coduser  = global_v_coduser
                 where codcomp  = p_codcomp
                   and dteyreap = p_dteyreap
                   and grade    = v_grade ;
            end if;
      ...
      if param_msg_error is null then
          ...
          if p_codappr is not null and p_dteappr is not null then
              begin
                  update tappraism set codappr = p_codappr, dteappr = p_dteappr, coduser  = global_v_coduser
                  where codcomp  like p_codcomp||'%' and dteyreap = p_dteyreap;
              end;
              commit;
          end if;
          -- >>
...
--#5552
  ...
  procedure get_detail(json_str_input in clob, json_str_output out clob) as
    ...
    cursor c_tappraism is
      select grade,codempid,amtsalo,amtmidsal,pctsal,
             amtbudg,amtsaln,amtceiling,amtminsal,amtover,
             codcomp,dteyreap
        from tappraism
       where codcomp  like p_codcomp||'%'
         and dteyreap = p_dteyreap
  ...
  begin
    ...
    for i in c_tappraism loop
        ...
        if v_flgpass then
            ...
            if v_zupdsal = 'Y' then
                ...
                v_amtminsal     := (stddec(i.amtmidsal,i.codempid,v_chken));
                v_overmax  := 0;
                v_belowmin := 0;
    ...
    if v_flgdata = 'N' then
        param_msg_error := get_error_msg_php('HR2055', global_v_lang, 'tappraism');
        json_str_output := get_response_message(null, param_msg_error, global_v_lang);
