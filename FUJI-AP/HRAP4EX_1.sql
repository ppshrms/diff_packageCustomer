--------------------------------------------------------
--  DDL for Package Body HRAP4EX
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "HRAP4EX" is
...
--<<user25 Date : 14/09/2021 3. AP Module #4327
...
-->>user25 Date : 14/09/2021 3. AP Module #4327
  ...
  procedure gen_data2(json_str_output out clob) is
    ...
    v_label1        varchar2(500);

    cursor c1 is
      select  grade, codkpino, kpides, wgt, target,  kpivalue, achieve, mtrfinish, nvl(qtyscor,0) qtyscor, stakpi
        from  tkpidph  b
       where  dteyreap      = b_index_dteyreap
         and  numtime       = b_index_numtime
         and  codcomp       like b_index_codcomp || '%'
    ...
    if v_flgdata = 'Y' then
      ...
      for i in c1 loop
        ...
        if flgpass then
         ...
         -->>user25 Date : 14/09/2021 3. AP Module #4327
          obj_data.put('grade',i.grade||'-'||v_desgrade);
          obj_data.put('kpino',i.codkpino);
          obj_data.put('desc_kpi',i.kpides);
          obj_data.put('weight',i.wgt);
          obj_data.put('target',i.target);
          obj_data.put('volumn',to_char(nvl(i.kpivalue,0),'fm999,999,999,990.00'));
          obj_data.put('port',i.achieve);
          obj_data.put('volumn2',to_char(nvl(i.mtrfinish,0),'fm999,999,999,990.00'));
...
--      obj_data.put('volumn','');
--      obj_data.put('port','');
--      obj_data.put('volumn2',get_label_name('HRAP4EX1', global_v_lang, '100'));
...
--      obj_row.put(to_char(v_rcnt-1),obj_data);
  ...
  procedure gen_graph is
    ...
    v_codapp    ttemprpt.codapp%type := 'HRAP4EX';
...
cursor c1 is
    select  grade, codkpino, kpides, target,  kpivalue, achieve, mtrfinish, qtyscor, stakpi
      from  tkpidph  b
     where  dteyreap      = b_index_dteyreap
       and  numtime       = b_index_numtime
       and  codcomp       = b_index_codcomp
...
begin
    ...
    if v_flgdata = 'Y' then
        ...
        v_item1  := '';
        v_item14 := '';
        v_item31 := get_label_name('HRAP4EX1', global_v_lang, '140');
        v_seq_grd := 0;
        for i in c1 loop
            flgpass := secur_main.secur7(b_index_codcomp,global_v_coduser);
            if flgpass then
