...
--------------------------------------------------------
  ...
  procedure gen_index (json_str_output out clob) is
    ...
    cursor c_tbonuspy is
        select codcomp,typpayroll,codbon,dteyreap,numtime,
               codpay,numperiod,dtemthpay,dteyrepay
          from tbonuspy
  ...
  begin
    ...
    for i in c_tbonuspy loop
        ...
        begin
            select count(codempid),sum(stddec(amtnbon,codempid,v_chken))
              into v_numemp,v_amtnbon
              from tbonus
             where dteyreap     = i.dteyreap
               and numtime      = i.numtime
               and codbon       = i.codbon
               and codcomp      like i.codcomp || '%'
               and typpayroll   = i.typpayroll
               and numperiod    = i.numperiod
               and dtemthpay    = i.dtemthpay
               and dteyrepay    = i.dteyrepay
               and nvl(flgtrnpy,'N')   = 'Y'
        ...
        obj_data.put('amtbon', nvl(v_amtnbon,0));
  ...
  begin
    ...
    if v_chkbonus = 'N' then
        param_msg_error := get_error_msg_php('HR2010', global_v_lang, 'TBONUS');
        return;
    end if;
    ...
    if param_msg_error is null then
      ...
      for i in 0..param_json.get_size-1 loop
        ...
        if param_msg_error is null then
            ...
            obj_data.put('desc_codpay', get_tinexinf_name(p_codpay,global_v_lang)); -- softberry || 11/08/2023 || #9469 || obj_data.put('desc_codpay', p_codpay||' - '||get_tinexinf_name(p_codpay,global_v_lang));
  ...
  procedure ins_tothinc (v_codempid   in temploy1.codempid%type,
    ...
    cursor c_tothinc is
        select codempid,stddec(amtpay,codempid,v_chken) amtpay,rowid
          from tothinc
  ...
  begin
	...
	if not v_flgfound then
		v_amtpaynet := stdenc(v_amtnbon,v_codempid,v_chken);
        insert into tothinc(
