--------------------------------------------------------
--  DDL for Package Body HRAPATE
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "HRAPATE" is
-- last update: 25/05/2018 12:23
  procedure initial_value(json_str in clob) is
	...
	procedure getDestroy (json_str_input in clob, json_str_output out clob) is
		...
		obj_data		json_object_t;
		v_rcnt			number;
    p_codcompy      tapalert.codcompy%type;
		p_mailalno		tapalert.mailalno%type;
		p_dteeffec		tapalert.dteeffec%type;
	...
	begin
		...
		for i in 0..arr_destroy.get_size-1 loop
			...
			begin
				delete from tapalert where codcompy = p_codcompy and mailalno = p_mailalno and dteeffec = p_dteeffec;
				delete from tapparam where codcompy = p_codcompy and mailalno = p_mailalno and dteeffec = p_dteeffec;
				delete from tapasign where codcompy = p_codcompy and mailalno = p_mailalno and dteeffec = p_dteeffec;
				delete from tmailrap where codcompy = p_codcompy and mailalno = p_mailalno and dteeffec = p_dteeffec;
			end;
		end loop;
  ...
  function get_seqno (v_codcompy varchar2, v_mailalno varchar2, v_dteeffec date) return number is
    ...
    p_codsend           tapalert.codsend%type;
    p_code              tapalert.syncond%type;
    p_statement         tapalert.statement%type;
		v_rcnt			        number := 0;
    ...
    arr_report          json_object_t;
		...
		p_ffield		        tapparam.ffield%type;
    p_descript          tapparam.descript%type;
		p_codtable		      tapparam.codtable%type;
		p_fparam		        tapparam.fparam%type;
		p_flgdesc	          tapparam.flgdesc%type;
    p_flgdesc_b         boolean;
		p_numseq		        tapparam.numseq%type;
		v_numseq		        number := 0;
		p_flgDelete		      boolean;

    p_pfield                tmailrap.pfield%type;
    p_pdesct                tmailrap.pdesct%type;
    v_flgappr               tapasign.flgappr%type;
    v_codcompap             tapasign.codcompap%type;
    v_codposap              tapasign.codposap%type;
    v_codempap              tapasign.codempap%type;
    v_message               tapasign.message%type;
...
--            param_msg_error := get_error_msg_php('HR3007',global_v_lang,get_label_name('HRAPATE2',global_v_lang,'320'));
          param_msg_error := get_error_msg_php('HR3007',global_v_lang);
          json_str_output := get_response_message(null,param_msg_error,global_v_lang);
          return;
      end if;
      ...
      if v_staemp = '9' then
          param_msg_error := get_error_msg_php('HR2101',global_v_lang,get_label_name('HRAPATE2',global_v_lang,'320'));
          json_str_output := get_response_message(null,param_msg_error,global_v_lang);
          return;
      elsif v_staemp = '0' then
          param_msg_error := get_error_msg_php('HR2102',global_v_lang,get_label_name('HRAPATE2',global_v_lang,'320'));
          json_str_output := get_response_message(null,param_msg_error,global_v_lang);
          return;
      end if;
      ...
      p_descript          := hcm_util.get_string_t(obj_row, 'paramdesc');
            ...
            if p_flgDelete then
                ...
                begin
                    ...
                    if sql%rowcount = 0 then
                        ...
                        insert into tapparam (codcompy, mailalno,dteeffec,numseq,fparam,ffield,descript,
        ...
        begin
            select 'x'
             ...
             where codcompy = p_codcompy
               and mailalno = p_mailalno
               and dteeffec = p_dteeffec
               and rownum = 1--user37 #7444 3. AP Module 27/01/2022
        ...
        begin
			INSERT INTO tapalert (codcompy,mailalno,dteeffec,typemail,flgeffec,subject, message,syncond,codsend,qtydayb, qtydayr,statement,codcreate,coduser)
		...
		if arr_assign is not null then
            ...
            if v_flgappr = '3' then
                ...
                if v_staemp = '9' then
                    param_msg_error := get_error_msg_php('HR2101',global_v_lang,get_label_name('HRAPATE2',global_v_lang,'360'));
                    json_str_output := get_response_message(null,param_msg_error,global_v_lang);
                    return;
                elsif v_staemp = '0' then
                    param_msg_error := get_error_msg_php('HR2102',global_v_lang,get_label_name('HRAPATE2',global_v_lang,'360'));
                    json_str_output := get_response_message(null,param_msg_error,global_v_lang);
                    return;
                end if;
            end if;
						...
						update tapasign
						 ...
						 where codcompy = p_codcompy
                           ...
                           and seqno = v_seqno;
						if sql%rowcount = 0 then
							insert into tapasign (codcompy, mailalno, dteeffec, seqno,flgappr,codcompap,codposap,codempap,message,coduser,codcreate)
        ...
        if v_count_assign = 0 then
            param_msg_error := get_error_msg_php('HR2045',global_v_lang,get_label_name('HRAPATE2',global_v_lang,'360'));
            json_str_output := get_response_message(null,param_msg_error,global_v_lang);
            rollback;
            return;
        end if;
            ...
            update tmailrap
             ...
             where codcompy = p_codcompy
               ...
               and numseq = p_numseq;
                  ...
                  insert into tmailrap (codcompy, mailalno, dteeffec, pfield,
        ...
        if v_count_report = 0 then
            param_msg_error := get_error_msg_php('HR2045',global_v_lang,get_label_name('HRAPATE2',global_v_lang,'370'));
            json_str_output := get_response_message(null,param_msg_error,global_v_lang);
            rollback;
            return;
        end if;
    ...
    v_row			          number := 0;
    v_desc			        varchar2(200 char);
    v_message_display   tapalert.message%type;
    v_dteeffec          date;
    ...
    p_mailalno          tmailrap.mailalno%type;
    p_flgDelete         boolean;
    p_flgdesc           tmailrap.flgdesc%type;
    p_numseq            tmailrap.numseq%type;
    p_pfield            tmailrap.pfield%type;
    p_pdesct            tmailrap.pdesct%type;
	begin
		initial_value(json_str_input);
	...
	begin
		...
		if v_number_mail > 0 then
			case p_typemail
				when '1' then alertmsg_ap.gen_typemail1(p_codcompy,'N',p_mailalno,p_dteeffec,'N');
				when '2' then alertmsg_ap.gen_typemail2(p_codcompy,'N',p_mailalno,p_dteeffec,'N');
			end case;
