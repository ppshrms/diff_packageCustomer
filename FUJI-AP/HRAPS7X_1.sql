                    ...
                    ----------------
                     ...
                     if v_amtmaxsa <> 0 then
                        if nvl(v_job_grade,'####') <> i.jobgrade then
                            ...
                            v_chk_min := 'N';
                            for k in 1..5 loop
                                v_amtsal_st(k) :=  (((b_index_perc_st(k))/100) * v_amtmaxsa) + v_amtmaxsa;
                                v_amtsal_en(k) :=  (((b_index_perc_en(k))/100) * v_amtmaxsa) + v_amtmaxsa;
                                ...
                                if b_index_perc_st(k) is not null and nvl(v_emp,0) > 0 then
                                   -- v_rcnt := v_rcnt+1;
                                    ...
                                    v_amtsal_st(k) :=  (((b_index_perc_st(k))/100) * v_amtmaxsa) + v_amtmaxsa; --- Redmine #9811
                                    v_amtsal_en(k) :=  (((b_index_perc_en(k))/100) * v_amtmaxsa) + v_amtmaxsa; --- Redmine #9811
                                    obj_data.put('ragesaldefne',v_amtsal_st(k));
                                    obj_data.put('leveldefind',v_amtsal_en(k));

                                    obj_data.put('ranksalmin',(((b_index_perc_st(k))/100) * v_amtmaxsa) + v_amtmaxsa); --- Redmine #9811
                                    obj_data.put('ranksalmax',(((b_index_perc_en(k))/100) * v_amtmaxsa) + v_amtmaxsa); --- Redmine #9811
                                    ...
                                    ---------------
                                        ...
                                        if  v_emp > 0  then
                                            ...
                                            if v_chk_min <> 'Y' then
                                                v_chk_min := 'Y';
                                                v_min := (((b_index_perc_st(k))/100) * v_amtmaxsa) + v_amtmaxsa; --- Redmine #9811
                                                v_max := (((b_index_perc_en(k))/100) * v_amtmaxsa) + v_amtmaxsa; --- Redmine #9811
                                                v_tpm_count := v_rcnt;
                                            else
                                                v_max := (((b_index_perc_en(k))/100) * v_amtmaxsa) + v_amtmaxsa; --- Redmine #9811
                                            end if;
        ...
        if flgpass then
          ...
          if v_summth between b_index_amtsal_st and b_index_amtsal_en then
              ...
              if b_index_amt_maxsal = 0 then
                  ...
                  obj_data.put('pctceisal',to_char((v_summth/b_index_amt_maxsal),'fm990.00'));
              end if;
