  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "HRAPS9B_BATCH" as
    ...
    procedure start_process (  p_codapp  in varchar2,
        ...
        cursor c_ttbonparc is
            select ratecond,ratebon,amttbon,numseq numcond
              from ttbonparc
      ...
      begin
          select codincom1,codincom2,codincom3,codincom4,codincom5,
           ...
           where codcompy = b_var_codcompy
             and dteeffec = (select max(dteeffec) from tcontpms where codcompy = b_var_codcompy );
      ...
      for r_temploy in c_temploy loop
          ...
          if v_flgsecu then
              ...
              if v_boncond is not null then
                  ...
                  ---v_process = 'HRAPS9B'
                  v_bonconds := replace(v_bonconds,'V_HRAPS9.CODCOMP',''''||r_temploy.codcomp||'''');
                  v_bonconds := replace(v_bonconds,'V_HRAPS9.AGEPOS',round(months_between(sysdate,r_temploy.dteempmt),2) );
                  v_bonconds := replace(v_bonconds,'V_HRAPS9.TYPEMP',''''||r_temploy.typemp||'''');
                  v_bonconds := replace(v_bonconds,'V_HRAPS9.TYPPAYROLL',''''||r_temploy.typpayroll||'''');
                  v_bonconds := replace(v_bonconds,'V_HRAPS9.CODPOS',''''||r_temploy.codpos||'''');
                  v_bonconds := replace(v_bonconds,'V_HRAPS9.JOBGRADE',''''||r_temploy.jobgrade||'''');
                  v_bonconds := replace(v_bonconds,'V_HRAPS9.CODEMPMT',''''||r_temploy.codempmt||'''');
                  ---v_process = 'HRAP54B'
                  v_bonconds := replace(v_bonconds,'V_TEMPLOY.CODCOMP',''''||r_temploy.codcomp||'''');
                  v_bonconds := replace(v_bonconds,'V_TEMPLOY.AGEPOS',round(months_between(sysdate,r_temploy.dteempmt),2) );
              ...
              if v_flgfound <> 0 then
                  if r_temploy.dteempmt < v_dteend then
                      ...
                      if v_typbon = '1' then
                          begin
                              select grdap,pctdbon,flgbonus,qtytotnet
                                into v_grdap,v_pctdbon,v_flgbonus,v_qtytotnet
                                from tappemp
                          ...
                          if v_process = 'HRAPS9B' then
                              for r_ttbonparc in c_ttbonparc loop
                                  ...
                                  if r_ttbonparc.ratecond is not null then
                                      ...
                                      v_ratecondt := replace(v_ratecondt,'V_HRAPS9.NUMLVL', r_temploy.numlvl );
                                      v_ratecondt := replace(v_ratecondt,'V_HRAPS9.CODPOS',''''||r_temploy.codpos||'''');
                                      v_ratecondt := replace(v_ratecondt,'V_HRAPS9.JOBGRADE',''''||r_temploy.jobgrade||'''');
                                      v_ratecondt := replace(v_ratecondt,'V_HRAPS9.GRADE',''''||v_grdap||'''');
                                      v_ratecondt := replace(v_ratecondt,'V_HRAPS9.SCORE', v_qtytotnet );
                                      v_ratecondt := replace(v_ratecondt,'V_HRAPS9.AGEPOS',( months_between(sysdate,r_temploy.dteempmt)));
                                      v_ratecondt := replace(v_ratecondt,'V_HRAPS9.AGE_POS',( months_between(sysdate,r_temploy.dteempmt)));
                                      v_ratecondt := 'select count(*) from temploy1 where '||v_ratecondt||' and codempid ='''||r_temploy.codempid||'''' ;
                                      v_flgfound  := execute_qty(v_ratecondt) ;
                                  end if;
                              ...
                              for r_tbonparc in c_tbonparc loop
                                  ...
                                  if v_flgfound <> 0 then
                                      v_ratebon := r_tbonparc.ratebonc;
                                      v_amttbon := r_tbonparc.amttbonc;
      ...
      for r_temploy in c_temploy loop
          ...
          if v_flgsecu then
              ...
              if v_boncond is not null then
                  v_bonconds := v_boncond;
                  v_bonconds := replace(v_bonconds,'V_TEMPLOY.CODCOMP',''''||r_temploy.codcomp||'''');
                  v_bonconds := replace(v_bonconds,'V_TEMPLOY.AGE_POS',round(months_between(sysdate,r_temploy.dteempmt),2) );
                  v_bonconds := replace(v_bonconds,'V_TEMPLOY.AGEPOS',round(months_between(sysdate,r_temploy.dteempmt),2) );
                  v_bonconds := replace(v_bonconds,'V_TEMPLOY.TYPEMP',''''||r_temploy.typemp||'''');
                  v_bonconds := replace(v_bonconds,'V_TEMPLOY.TYPPAYROLL',''''||r_temploy.typpayroll||'''');
                  v_bonconds := replace(v_bonconds,'V_TEMPLOY.CODPOS',''''||r_temploy.codpos||'''');
