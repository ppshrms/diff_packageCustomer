...
--<< user22 : 06/06/2024 :Redmine No.10813 || 
    cursor c_grade is
      select decode(b_index_typeof,'A',grade,'O',numcond)  grade_,codbon,numtime,numcond -- user22 : 06/06/2024 :Redmine No.10813 || select decode(b_index_typeof,'A',grade,'O',numcond)  grade,codbon,numtime
        from tbonus a
       ...
       group by decode(b_index_typeof,'A',grade,'O',numcond) ,codbon,numtime,numcond
       order by decode(b_index_typeof,'A',grade,'O',numcond);
    /*cursor c_grade is
...
-->> user22 : 06/06/2024 :Redmine No.10813 ||       
    ...
    if v_flgdata = 'Y' then
        for i in c_grade loop
           -- user22 : 06/06/2024 :Redmine No.10813 || flgpass := secur_main.secur7(i.codcomp,global_v_coduser);
           -- user22 : 06/06/2024 :Redmine No.10813 || if flgpass then
               v_flgsecu := 'Y';
                v_rcnt := v_rcnt+1;
                obj_data := json_object_t();
                obj_data.put('coderror', '200');
                obj_data.put('grade',i.grade_);
                -- ***
                v_cs := 0;
                if b_index_dteyear1 is not null then
                  v_cs := v_cs+1;
                  get_data_sumary(b_index_dteyear1, i.grade_, i.numcond, v_desc, v_qty_policy, v_qty_actual);-- user22 : 06/06/2024 :Redmine No.10813 ||  get_data_sumary(b_index_dteyear1, i.grade, i.numcond, v_desc, v_qty_policy, v_qty_actual);
                  v2_desc := v_desc;
                ...
                if b_index_dteyear2 is not null then
                  v_cs := v_cs+1;
                  get_data_sumary(b_index_dteyear2, i.grade_, i.numcond, v_desc, v_qty_policy, v_qty_actual);-- get_data_sumary(b_index_dteyear2, i.grade, i.numcond, v_desc, v_qty_policy, v_qty_actual);
                  if v_desc = '***************'  then
                    v_desc := v2_desc;
                  end if;
                  obj_data.put('desc_grade',v_desc);
                ...
                if b_index_dteyear3 is not null then
                  v_cs := v_cs+1;
                  get_data_sumary(b_index_dteyear3, i.grade_, i.numcond, v_desc, v_qty_policy, v_qty_actual);-- get_data_sumary(b_index_dteyear3, i.grade, i.numcond, v_desc, v_qty_policy, v_qty_actual);
                  if v_desc = '***************'  then
                    v_desc := v2_desc;
                  end if;
                  obj_data.put('desc_grade',v_desc);
            ...
            -- user22 : 06/06/2024 :Redmine No.10813 || end if;-- if flgpass
        end loop;-- i
    end if;--v_flgdata = 'Y'
...
--<< user22 : 06/06/2024 :Redmine No.10813 || 
    cursor c1 is
      select decode(b_index_typeof,'A',grade,'O',numcond)  grade_
        from tbonus a
       where codcomp    like b_index_codcomp||'%'
         and codbon     = b_index_codbon
         and numtime    = b_index_numtime
         ...
         and exists (select b.codcomp 
                      ...
                      where b.coduser = global_v_coduser
                        and a.codcomp like b.codcomp||'%')
       group by decode(b_index_typeof,'A',grade,'O',numcond)
       order by decode(b_index_typeof,'A',grade,'O',numcond);
...
-->> user22 : 06/06/2024 :Redmine No.10813 || 
    ...
    cursor c_dteyreap is
      select    dteyreap
        from    tbonus a -- user22 : 06/06/2024 :Redmine No.10813 || from    tbonus
       where    codcomp   like b_index_codcomp||'%'
         and    codbon    = b_index_codbon
         and    numtime   = b_index_numtime
         and    (dteyreap = nvl(b_index_dteyear1,dteyreap)
                ...
                if  v_cs = v_cs1 then
                  ...
                  if b_index_typeof = 'A' then --Grade ?????????????
                        begin
                            select  ratebon
                              into  v_qty_policy
                              from  ttbonpard a-- user22 : 06/06/2024 :Redmine No.10813 || from  ttbonpard
--                             where  codcomp = i.codcomp
                             where codcomp  like b_index_codcomp||'%'-- user22 : 06/06/2024 :Redmine No.10813 || where  codcomp like hcm_util.get_codcomp_level(i.codcomp,1)||'%'
                               and codbon   = b_index_codbon
                               and numtime  = b_index_numtime
                      ...
                      --v_itemXX  := get_tstdis_name(i.codcomp,v_dteyreap,i.grade,global_v_lang);
                        ...
                        begin
                            select ratebon,ratecond
                              ...
                              from ttbonparc a-- user22 : 06/06/2024 :Redmine No.10813 || from  ttbonparc
                             where codcomp     like b_index_codcomp||'%'-- user22 : 06/06/2024 :Redmine No.10813 || where  codcomp     like hcm_util.get_codcomp_level(i.codcomp,1)||'%'
                               and codbon      = b_index_codbon
                               and numtime     = b_index_numtime
                               and dteyreap    = j.dteyreap--v_dteyreap
                         ...
                         begin
                            select nvl(sum(nvl(qtybon,0))/count(codempid),0)
                             ...
                             where codcomp     like b_index_codcomp||'%'-- user22 : 06/06/2024 :Redmine No.10813 || where codcomp  like hcm_util.get_codcomp_level(i.codcomp,1)||'%'
                               ...
                               and numtime     = b_index_numtime
                               and dteyreap    = j.dteyreap--v_dteyreap
                               and decode(b_index_typeof,'A',grade,'O',numcond,grade) = i.grade_
--<< user22 : 06/06/2024 :Redmine N.10813 ||                  
                               and exists (select b.codcomp 
                                             from tusrcom b
                                            where b.coduser = global_v_coduser
                                              and a.codcomp like b.codcomp||'%')
...
-->> user22 : 06/06/2024 :Redmine No.10813 ||                                    
            ...
            ----------Insert ttemprpt
                ...
                exception when dup_val_on_index then
                 ...
                 v_numseq := v_numseq + 1;
             end if; -- v_cs = v_cs1 then
            end loop;--loop k
       end loop; --loop j
       -- user22 : 06/06/2024 :Redmine No.10813 || end if; --end if;--if flgpass
    end loop;--loop i
    commit;
  end if;-- if v_flgdata
  exception when others then
     ...
     if b_index_typeof = 'A' then --Grade ?????????????
           begin
              select ratebon
                into v_qty_policy
                from ttbonpard a -- user22 : 06/06/2024 :Redmine No.10813 || from  ttbonpard
               where codcomp  like b_index_codcomp||'%'-- user22 : 06/06/2024 :Redmine No.10813 || where  codcomp  like hcm_util.get_codcomp_level(b_index_codcomp,1)||'%'
                 and codbon   = b_index_codbon
                 and numtime  = b_index_numtime
                 and dteyreap = v_year
                 and grade    = v_grade
      ...
      begin
          select nvl(sum(nvl(qtybon,0))/count(codempid),0)
           ...
           where codcomp   like b_index_codcomp||'%'-- user22 : 06/06/2024 :Redmine No.10813 || where  codcomp like hcm_util.get_codcomp_level(b_index_codcomp,1)||'%'
             ...
             and numtime   = b_index_numtime
             and dteyreap  = v_year
             and((b_index_typeof = 'A' and grade   = v_grade)
              or (b_index_typeof = 'O' and numcond = v_numcond))
             --and decode(b_index_typeof,'A',grade,'O',numcond) = v_grade
             and exists (select b.codcomp 
                           from tusrcom b
                          where b.coduser = global_v_coduser
                            and a.codcomp like b.codcomp||'%');                   
